...
<%@ page import="com.tradevan.util.report.RptFR070W" %>								          
...
<%
   ...
   if(session.getAttribute("muser_id")==null){//session timeout		
      System.out.println("FR070W login timeout");   
   		...
   		if(!CheckPermission(request)){//無權限時,導向到LoginError.jsp
	    	...
	    	if(act.equals("Qry")){
   			...
   			if(act.equals("view") || act.equals("download")){
   				try{		    
	    			actMsg=RptFR070W.createRpt(S_YEAR,S_MONTH,Unit);	    
...
<%!
    ...
    private final static String QryPgName="/pages/FR070W_Qry.jsp";        
    private final static String RptCreatePgName="/pages/FR070W_Excel.jsp";        
    private final static String LoginErrorPgName="/pages/LoginError.jsp";
    private boolean CheckPermission(HttpServletRequest request){//檢核權限    	        		
    	    boolean CheckOK=false;
    	    HttpSession session=request.getSession();            
            Properties permission=( session.getAttribute("FR070W")==null ) ? new Properties() : (Properties)session.getAttribute("FR070W");				                
            if(permission==null){
              System.out.println("FR070W.permission==null");
            }else{
               System.out.println("FR070W.permission.size="+permission.size());
