<%
/*
  *   created on 95.1.4  by lilic0c0 4183  
  *  
  */ 
%>
...
<%
	System.out.println("FR030WB_Qry.jsp Program Start...");
	//2005 09 15 fix timeout by 1495 取得session資料(取得到表示還未timeout),取得成功時,才繼續往下執行 ======================
	RequestDispatcher rd = null;
	
	//判斷有沒有session timeout
	if(session.getAttribute("muser_id") == null){
 	...
 	//取得FR035W的權限
	Properties permission = ( session.getAttribute("FR030WB")==null ) ? new Properties() : (Properties)session.getAttribute("FR030WB");
	
	if(permission == null){
       System.out.println("FR030WB_Qry.permission == null");
    }else{
       System.out.println("FR030WB_Qry.permission.size ="+permission.size());
    }
...
<% //取得參數===================================================
	...
	Calendar rightNow = Calendar.getInstance();
   	...
   	int MONTH = rightNow.get(Calendar.MONTH)+1;   //月份以0開始故加1取得實際月份;
%>
...
function doSubmit(){
   if(!confirm("本項報表會執行3~5秒，是否確定執行？"))
   		return;
   this.document.forms[0].action = "/pages/FR030WB_Excel.jsp";
                ...
                <td width="*">
                	<%
                		if(bank_type.equals("6")){
                			out.print("<font color=\"#336600\" size=4><center>台灣區農會信用部支票存款戶數與餘額總表</center></font>");
                		} else{
                			out.print("<font color=\"#336600\" size=4><center>台灣區漁會信用部支票存款戶數與餘額總表</center></font>");
                		}	 
					%>                
                </td>
                <td width="100"><img src="images/banner_bg1.gif" width="100" height="17"></td>
          ...
          <td>
              ...
              <tr>
                ...
                <table width="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#E9F4E3">
                    ...
                    <tr>
                      <td bgcolor="#B0D595" class="sbody"> <div align="right">
                          <!--input type='radio' name="act" value='view' checked>檢視報表-->
                      	  ...
                      	  %>
                      	 	 <a href="javascript:doSubmit()" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image411','','images/bt_execb.gif',1)">
                      	 	 <img src="images/bt_exec.gif" name="Image411" width="66" height="25" border="0" id="Image41"></a>
                      	  <%}%>
                    ...
                    <tr>
                      <td>
                      	  <img src="images/2_icon_01.gif" width="16" height="16" align="absmiddle"><span class="mtext">查詢年度 :</span>
...
<%
	System.out.println("FR030WB_Qry.jsp Program End...");
%>