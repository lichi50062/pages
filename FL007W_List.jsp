...
<%
    ...
    String ag_Rt_DocNo = Utility.getTrimString(dataMap.get("ag_Rt_DocNo")) ; 
...
<tr class="sbody" >
       ...
       <td bgcolor="#EBF4E1" height="1">
       <input type="text" name="ag_Rt_DocNo" value='<%=ag_Rt_DocNo %>' size="49" maxlength='10'>
</tr>  
</Table>
...
<% if(!"".equals(ex_Type)) {%>
    ...
    <tr class="sbody" bgcolor="#BFDFAE">
    	<td width="7%" rowspan='2'>機構代號</td>
    	<td width="19%" rowspan='2'>機構名稱</td>
      	<td width="10%" rowspan='2'><%if("FEB".equals(ex_Type)){%>檢查報告編號<%}else if("AGRI".equals(ex_Type)){%>查核季別<%}else{%>訪查日期<%} %></td>
        <td width="10%" rowspan='2'>收文日期</td>        
      	<td width="10%" rowspan='2'>收文文號</td>
      	<td colspan='2' align='center'>辦理依據</td>
      	<td width="12%" rowspan='2'>繳款補貼息日期</td>
      	<td width="14%" rowspan='2'>繳款補貼息金額(元)</td>
    </tr>
    <tr class="sbody" bgcolor="#BFDFAE">
      	<td >發文日期</td>
      	<td >發文文號</td>
    ...
    <tr class="sbody" bgcolor='<%=(i % 2 == 0)?"#EBF4E1":"#FFFFCC"%>'>
      ...
      <td><%=bean.getValue("ag_rt_doc_date") != null ? (String)bean.getValue("ag_rt_doc_date") : "&nbsp;"%></td>
      <td>
	      <a href=FL007W.jsp?act=Edit&bank_No=<%=bean.getValue("bank_no")%>&ex_Type=<%=bean.getValue("ex_type")%>&ex_No=<%=bean.getValue("ex_no")%>&ag_Rt_DocNo=<%=bean.getValue("ag_rt_docno")%>>
	      	<%=bean.getValue("ag_rt_docno") != null ? bean.getValue("ag_rt_docno") : "&nbsp;"%>
	      </a>&nbsp;</td>
      <td><%=bean.getValue("doc_date") != null ? (String)bean.getValue("doc_date") : "&nbsp;"%></td>
      <td><%=bean.getValue("docno") != null ? (String)bean.getValue("docno") : "&nbsp;"%></td>
      <td><%=bean.getValue("pay_date") != null ? (String)bean.getValue("pay_date") : "&nbsp;"%></td>
      <td><%=bean.getValue("pay_amt_sum") != null ? Utility.setCommaFormat(bean.getValue("pay_amt_sum").toString()) : "&nbsp;"%></td>
    </tr>
    <%        
      }
    }else{%>            
