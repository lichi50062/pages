...
<%
    ...
    String SortList = "";
    String S_YEAR = "";//99.12.30 add      
	String S_MONTH = "";//99.12.30 add     
    String CANCEL_NO = "";
	...
	try{
			...
			p.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"WLX04.length"));
			//====================================================================================================
            ...
            String table = " WLX04 LEFT JOIN (select * from bn01 where m_year=?)BN01 ON WLX04.bank_no = BN01.bank_no ";//查詢table
            String order = "";//排序欄位
            String sqlCmd="";
            paramList.add(wlx01_m_year);//bn01用
            //金融機構代號=============================================================
            if(BankList_data != null && BankList_data.size() != 0){
               selectBank_no += " WLX04.bank_no IN (";
            ...
            for(i=0;i<btnFieldList_data.size();i++){
               ...
               //選取欄位         			 
			   if(((String)((List)btnFieldList_data.get(i)).get(0)).equals("WLX04.POSITION_CODE")){
			       column += " WLX04.POSITION_CODE,cdshareno.cmuse_name as position_code_name";			   	  
			   }else{
               ...
               //條件式跟table=============================================================================
               if(((String)((List)btnFieldList_data.get(i)).get(0)).equals("WLX04.POSITION_CODE")){               	  
               	  table += " LEFT JOIN cdshareno on WLX04.position_code = cdshareno.cmuse_id and cdshareno.cmuse_div='008'";
               }
               //=========================================================================================
            }            
            ...
            if(!position_code.equals("ALL")){
               ...
               condition += " and WLX04.position_code = ? ";
            }
            //99.01.08 add 卸任與否============================================================================
            ...
            /*  select BN01.BANK_NO,BN01.BANK_NAME,WLX04.POSITION_CODE,
	   					cdshareno.cmuse_name as position_code_name,
	   					WLX04.RANK,WLX04.ID,WLX04.NAME,WLX04.SEX,
	   					WLX04.BIRTH_DATE,WLX04.PASSPORT_AREA,WLX04.PASSPORT_NO,
	   					WLX04.INDUCT_DATE,WLX04.APPOINTED_NUM,WLX04.ABDICATE_DATE,
	   					WLX04.PERIOD_START,WLX04.PERIOD_END,WLX04.DEGREE,
	   					WLX04.BACKGROUND,WLX04.TELNO
				from WLX04 LEFT JOIN (select * from bn01 where m_year=99)BN01 ON WLX04.bank_no = BN01.bank_no 
	 				  	   LEFT JOIN cdshareno on WLX04.position_code = cdshareno.cmuse_id and cdshareno.cmuse_div='008'
				where WLX04.bank_no IN ('6030016')
				and   BN01.bn_type <> '2'	   
				and abdicate_code <> 'Y'      
  			*/
  			...
  			System.out.println("BR008W_Excel.sqlCmd="+sqlCmd);	   
			List dbData = DBManager.QueryDB_SQLParam(sqlCmd,paramList,"rank,birth_date,induct_date,apponited_num,abdicate_date,period_start,period_end");       
            //List dbData = DBManager.QueryDB(sqlCmd,"rank,birth_date,induct_date,apponited_num,abdicate_date,period_start,period_end");            
			String field = "";
			String tmpbank_no = "";
			String tmpbank_name = "";			
			...
			if(dbData == null || dbData.size() == 0){
                ...
                reportUtil.createCell( wb, row, ( short )1,"無理監事基本資料" ,noBorderDefaultStyle ); 
            ...
            for ( i = 0; i < dbData.size(); i ++) {                
                ...
                for(int j=0;j<btnFieldList_data.size();j++){
			        ...
			        if(field.equals("birth_date") || field.equals("induct_date")
			        || field.equals("abdicate_date") || field.equals("period_start") || field.equals("period_end")
			        ){//日期型態處理			        
			           ...
			           if(field.equals("rank") || field.equals("apponited_num")){//順位.本屆屆期			           
			              if(((DataObject)dbData.get(i)).getValue(field) != null){
			              ...
			              }else if(field.equals("bank_name")){//機構名稱   
			                 reportUtil.createCell( wb, row, ( short )(j+1),(String)((DataObject)dbData.get(i)).getValue(field) ,defaultStyle );					
