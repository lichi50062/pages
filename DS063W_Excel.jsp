...
<%
	...
	try{
			...
			sqlCmd.append("        		decode(field_loan_bal_amt,0,0,round(field_over3p_loan_amt / field_loan_bal_amt *100 ,2)) as field_over3p_loan_rate,");
			sqlCmd.append("        		decode(field_loan_bal_amt_sum,0,0,round(field_over3p_loan_amt / field_loan_bal_amt_sum *100 ,4)) as field_over3p_sum_rate,");
			sqlCmd.append("        		'0' as type_over3p_loan_amt,");
			sqlCmd.append("        		'0' as type_over3p_loan_cnt,");
			sqlCmd.append("        		'0' as type_loan_bal_amt,");
			sqlCmd.append("        		'4' as type_over3p_loan_rate,");
			sqlCmd.append("        		'4' as type_over3p_sum_rate ");
			...
			sqlCmd.append("               		round(sum(agri_loan.over3p_loan_cnt) /1,0) as field_over3p_loan_cnt,");
			sqlCmd.append("               		round(sum(agri_loan.over3p_loan_amt) /1,0) as field_over3p_loan_amt ");
            ...
            for(i=0;i<dbData.size();i++){            
                ...
                for(int cellIdx =4;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                     ...
                     if(!(amt_type.equals("4") || "over3p_loan_cnt".equals(tmpStr))){//該值不為利率或戶數時.再除以單位4捨五入
