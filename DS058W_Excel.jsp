<%
//108.04.30 add 報表格式轉換 by 2295
//111.04.08 調整排序欄位不是null時,才加入欄位 by 2295    
%>
...
<%
   ...
   String printStyle = "";//輸出格式 108.04.30 add 	     
   //輸出格式 108.04.30 add
   ...
   if(act.equals("view")){
      ...
      response.setHeader("Content-disposition","inline; filename=view."+printStyle);//108.04.30調整顯示的副檔名
   }else if (act.equals("download")){   
      response.setHeader("Content-Disposition","attachment; filename=download."+printStyle);//108.04.30調整顯示的副檔名
   }   
%>
<%
	...
	try{
			...
			if(session.getAttribute("S_YEAR") != null && !((String)session.getAttribute("S_YEAR")).equals("")){
		  		...
		  		lastYY = String.valueOf(Integer.parseInt(S_YEAR)-1);
			}
			//月
            ...
            if(SortList_data != null && SortList_data.size() != 0){
            	for(i=0;i<SortList_data.size();i++){
            		if(!"null".equals((String)((List)SortList_data.get(i)).get(0))){//111.04.08不是null時,才加入欄位
			...
			sqlCmd.append("        		field_over6m_loan_bal_cnt_last,");
			sqlCmd.append("        		field_over6m_loan_bal_amt_last,");
			sqlCmd.append("        		field_over6m_loan_bal_amt - field_over6m_loan_bal_amt_last as field_over6m_loan_bal_amt_diff,");
			sqlCmd.append("        		decode(field_over6m_loan_bal_amt_last,0,0,round((field_over6m_loan_bal_amt-field_over6m_loan_bal_amt_last) / field_over6m_loan_bal_amt_last *100 ,2))  as   field_over6m_loan_bal_amt_rate,");//--貸放餘額增加幅度%(A-B)/B
			sqlCmd.append("        		'0' as type_over6m_loan_bal_cnt,");
			sqlCmd.append("        		'0' as type_over6m_loan_bal_amt,");
			sqlCmd.append("        		'0' as type_over6m_loan_bal_cnt_last,");
			sqlCmd.append("        		'0' as type_over6m_loan_bal_amt_last,");
			sqlCmd.append("        		'0' as type_over6m_loan_bal_amt_diff,");
			sqlCmd.append("        		'4' as type_over6m_loan_bal_amt_rate ");
			sqlCmd.append(" 	   from (select agri_loan.m_year,agri_loan.m_month, agri_loan.bank_code ,");
			sqlCmd.append("               		decode(SUM(agri_loan.field_over6m_loan_bal_cnt),null,0,SUM(agri_loan.field_over6m_loan_bal_cnt)) as field_over6m_loan_bal_cnt,");//--當月-(6)貸放餘額-戶數
			sqlCmd.append("               		decode(SUM(agri_loan.field_over6m_loan_bal_amt),null,0,SUM(agri_loan.field_over6m_loan_bal_amt)) as field_over6m_loan_bal_amt,");//--當月-(7)貸放餘額
			sqlCmd.append("               		decode(SUM(agri_loan_last.field_over6m_loan_bal_cnt),null,0,SUM(agri_loan_last.field_over6m_loan_bal_cnt)) as field_over6m_loan_bal_cnt_last,");//--上月-(6)貸放餘額-戶數
			sqlCmd.append("               		decode(SUM(agri_loan_last.field_over6m_loan_bal_amt),null,0,SUM(agri_loan_last.field_over6m_loan_bal_amt)) as field_over6m_loan_bal_amt_last ");//--上月-(7)貸放餘額
			sqlCmd.append("        		   from ");
			sqlCmd.append("        				(select ? as m_year,? as m_month, agri_loan.bank_code,");        
			sqlCmd_paramList.add(S_YEAR);
			sqlCmd_paramList.add(S_MONTH);
			sqlCmd.append("                				round(sum(agri_loan.over6m_loan_bal_cnt) /1,0) as field_over6m_loan_bal_cnt,");//--(6)貸放餘額-戶數
			sqlCmd.append("                				round(sum(agri_loan.over6m_loan_bal_amt) /1,0) as field_over6m_loan_bal_amt ");//--(7)貸放餘額                       
			sqlCmd.append("         			   from agri_loan "); 
			sqlCmd.append("         		      where ( to_char(m_year * 100 + m_month) >= ? and to_char(m_year * 100 + m_month) <= ? ) ");
			sqlCmd_paramList.add(S_YEAR+"01");
			...
			sqlCmd.append("         			  group by agri_loan.bank_code ");
			...
			sqlCmd.append("         			   from agri_loan "); 
			sqlCmd.append("         			  where (to_char(m_year * 100 + m_month) >=? and to_char(m_year * 100 + m_month) <=? )");
			sqlCmd_paramList.add(lastYY+"01");
			sqlCmd_paramList.add(lastYY+S_MONTH);
			if(!"ALL".equals(loan_item)){
				sqlCmd.append("          			    and loan_item=? ");//--貸款項目別,若為全部,則不加入此條件    
				sqlCmd_paramList.add(loan_item);
			}
			...
			if(!"".equals(order)){	  
				...
				//SoryBy=asc/desc
				if( session.getAttribute("SortBy") != null && !((String)session.getAttribute("SortBy")).equals("") && !((String)session.getAttribute("SortBy")).equals("null")){//111.04.08不是null時,才加入欄位       	   
            ...
            for(i=0;i<dbData.size();i++){            
                ...
                for(int cellIdx =4;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                     ...
                     if(!(amt_type.equals("4") || "over6m_loan_bal_cnt".equals(tmpStr) || "over6m_loan_bal_cnt_last".equals(tmpStr))){//該值不為利率或戶數時.再除以單位4捨五入
            ...
            String filename = titleName+".xls";//108.04.30 add
            if(!printStyle.equalsIgnoreCase("xls")) {//108.04.30非xls檔須執行轉換	                
	  			rptTrans rptTrans = new rptTrans();	  			
	  			filename = rptTrans.transOutputFormat (printStyle,filename,""); 
	  			System.out.println("newfilename="+filename);	  			   
            };          
            
            FileInputStream fin = new FileInputStream(Utility.getProperties("reportDir")+System.getProperty("file.separator")+ filename);//108.04.30 fix 		 
