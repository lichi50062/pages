...
<% if(!"".equals(ex_Type)) {%>
    ...
    <tr class="sbody" bgcolor="#BFDFAE">
    	<td width="20%" >機構代號</td>
    	<td width="25%" >機構名稱</td>
      	<td width="20%" ><%if("FEB".equals(ex_Type)){%>檢查報告編號<%}else if("AGRI".equals(ex_Type)){%>查核季別<%}else{%>訪查日期<%} %></td>
        <td width="15%" >發文日期</td>        
      	<td width="20%" >發文文號</td>
    </tr>
    <%
    ...
    <tr class="sbody" bgcolor='<%=(i % 2 == 0)?"#EBF4E1":"#FFFFCC"%>'>
      ...
      <td>
	      <a href=FL008W.jsp?act=Edit&bank_No=<%=bean.getValue("bank_no")%>&ex_Type=<%=bean.getValue("ex_type")%>&ex_No=<%=bean.getValue("ex_no")%>&docNo=<%=bean.getValue("docno")%>>
