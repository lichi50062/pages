...
function doSubmit(report_no,cnd){
    if(cnd == 'createRpt'){
      ...
      //漁會科目代號新舊無法同時列印    
      if(document.RptOrderfrm.DS_bank_type.value == '7'){
        if(parseInt(document.RptOrderfrm.S_YEAR.value)<=102 && parseInt(document.RptOrderfrm.E_YEAR.value)>=103){
...
%>
	...
	for (var i =0; i < a.length; i ++){
		var j = a[i].split('+');
		document.RptOrderfrm.SortListDst.options[i] = new Option(j[1], j[0]);
	}
<%}%>
...
function getbtnFieldList(){
	...
	if(this.document.forms[0].BankList.value.indexOf("ALL") == -1){	   
	   //縣市別======================================================================================================
	   checkAdd = false;
       for(var k =0;k<document.RptOrderfrm.SortListDst.length;k++){			
	       if(document.RptOrderfrm.SortListDst.options[k].text == "縣市別"){		    
		      checkAdd = true;			       
	       }  
	   }  
	   if(checkAdd == false){	  
	      document.RptOrderfrm.SortListSrc.options[addCount] = new Option("縣市別", "wlx01.hsien_id");
	      addCount++;
	   }
	   //金融機構代號名稱============================================================================================
	   checkAdd = false;
	   for(var k =0;k<document.RptOrderfrm.SortListDst.length;k++){			
	       if(document.RptOrderfrm.SortListDst.options[k].text == "金融機構代號名稱"){		    
		    checkAdd = true;			       
	       }  
	   }    
       if(checkAdd == false){	  
          document.RptOrderfrm.SortListSrc.options[addCount] = new Option("金融機構代號名稱", "a01.bank_code");
          addCount++;
       }
       //年.月===================================================================================================
       checkAdd = false;
       for(var k =0;k<document.RptOrderfrm.SortListDst.length;k++){			
	       if(document.RptOrderfrm.SortListDst.options[k].text == "年月"){		    
		      checkAdd = true;			       
	       }  
	   } 	    
	   if(checkAdd == false){	  
	      document.RptOrderfrm.SortListSrc.options[addCount] = new Option("年月", "order_YYMM");
	      addCount++;
	   }	   
    }else{//已選的機構代號=ALL全部
       //縣市別小計============================================================================================
       checkAdd = false;
	   for(var k =0;k<document.RptOrderfrm.SortListDst.length;k++){			
	       if(document.RptOrderfrm.SortListDst.options[k].text == "縣市別小計"){		    
		      checkAdd = true;			       
	       }  
	   }  
	   if(checkAdd == false){	  
	      document.RptOrderfrm.SortListSrc.options[addCount] = new Option("縣市別小計", "cd01.FR001W_OUTPUT_ORDER");
	      addCount++;
	   }
	   //年.月===================================================================================================
	   checkAdd = false;
