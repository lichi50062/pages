...
<%@ page import="com.tradevan.util.report.RptFR071W" %>	
...
<%
   ...
   if(session.getAttribute("muser_id")==null){//session timeout		
      System.out.println("FR071W login timeout");   
   		...
   		if(!CheckPermission(request)){//無權限時,導向到LoginError.jsp
	    	...
	    	if(act.equals("Qry")){
   			...
   			if(act.equals("view") || act.equals("download")){
   				try{		    
	    			actMsg=RptFR071W.createRpt(S_YEAR,S_MONTH,Unit);	    
...
<%!
    ...
    private final static String QryPgName="/pages/FR071W_Qry.jsp";        
    private final static String RptCreatePgName="/pages/FR071W_Excel.jsp";        
    private final static String LoginErrorPgName="/pages/LoginError.jsp";
    private boolean CheckPermission(HttpServletRequest request){//檢核權限    	        		
    	    boolean CheckOK=false;
    	    HttpSession session=request.getSession();            
            Properties permission=( session.getAttribute("FR071W")==null ) ? new Properties() : (Properties)session.getAttribute("FR071W");				                
            if(permission==null){
              System.out.println("FR071W.permission==null");
            }else{
               System.out.println("FR071W.permission.size="+permission.size());
