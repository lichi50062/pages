...
<%
	System.out.println("@@TC56.jsp Start...");
	...
	if(session.getAttribute("muser_id") == null){
      System.out.println("TC56 login timeout");
	...
	if(!Utility.CheckPermission(request,report_no)){//無權限時,導向到LoginError.jsp
    	...
    	if(act.equals("new")){
           ...
           rd = application.getRequestDispatcher( ListPgName +"?act=List");
    	    ...
    	    List EXWARNIDFList = getQryResult(item_id);
    	    request.setAttribute("EXWARNIDFList",EXWARNIDFList);
    	    rd = application.getRequestDispatcher( ListPgName +"?act=Qry&item_id="+item_id);
    	}else if(act.equals("Edit")){
            List EXWARNIDF = getEXWARNIDF(item_id);
            request.setAttribute("EXWARNIDF",EXWARNIDF);
            if(EXWARNIDF.size() != 0){
               item_id = (String)((DataObject)EXWARNIDF.get(0)).getValue("item_id");
            }
            //設定異動者資訊======================================================================
			request.setAttribute("maintainInfo","select * from EXWARNIDF WHERE item_id='" + item_id+"'");
			//=======================================================================================================================
    	 	rd = application.getRequestDispatcher( EditPgName +"?act=Edit&item_id=");
...
<%!
	private final static String report_no = "TC56" ;
    ...
    //取得EXWARNIDF異常警訊項目資料
    private List getEXWARNIDF(String item_id){
    		//查詢條件
    		StringBuffer sqlCmd = new StringBuffer() ;
    		List paramList =new ArrayList() ;
    		sqlCmd.append(" SELECT * FROM EXWARNIDF  WHERE ITEM_ID=? " );
    ...
    //取得查詢結果
    private List getQryResult(String item_id){
    		//查詢條件
    		StringBuffer sqlCmd = new StringBuffer() ;
    		List paramList =new ArrayList() ;
    		System.out.println("Method getQryResult Start ..");
    		if(item_id.equals("")){
    			sqlCmd.append( "SELECT ITEM_ID,ITEM_NAME FROM EXWARNIDF ORDER BY ITEM_ID" );
    		}else{
				sqlCmd.append( "SELECT ITEM_ID,ITEM_NAME "
				   		+ "FROM EXWARNIDF WHERE  ITEM_ID =?"
				   		+ " ORDER BY ITEM_ID" );
				paramList.add(item_id) ;
			}
            List dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(),paramList,"update_date");
	...
	public String InsertDB(HttpServletRequest request,String lguser_id,String lguser_name) throws Exception{
		...
		String item_name=((String)request.getParameter("ITEM_NAME")==null)?" ":(String)request.getParameter("ITEM_NAME");
		...
		try {
			    sqlCmd.append("SELECT * FROM EXWARNIDF WHERE ITEM_ID='" + item_id + "'" );
			    paramList.add(item_id) ;
				...
				if (data.size() != 0){
			        ...
			        sqlCmd.append( "INSERT INTO EXWARNIDF VALUES (?"
					       + ",?"
					       + ",?"
					       + ",?"
					       + ",sysdate)");
			        paramList.add(item_id) ;
			        paramList.add(item_name) ;
	...
	private String UpdateDB(HttpServletRequest request,String lguser_id,String lguser_name) throws Exception{
		...
		try {
				...
				if (data.size() == 0){
				    ...
				    //=========================================================================
				    sqlCmd.append( "UPDATE EXWARNIDF SET "
				    	   + " ITEM_NAME=?"
					       + ",USER_ID=?"
					       + ",USER_NAME=?"
					       + ",UPDATE_DATE=sysdate"
		...
		try {
				...
				if (data.size() == 0){
					errMsg = errMsg + "此筆資料不存在無法刪除<br>";
				 	...
				 	sqlCmd.append("DELETE from EXWARNIDF where ITEM_ID =? " );
				 	paramList.add(item_id) ;
...
%>
<%System.out.println("@@TC56.jsp End...");%>