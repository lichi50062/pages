...
<body>
產生(農.漁會-各別機構)WR01_operation
<%
   System.out.println("=============產生WR01_Operation開始===========");
   ...
   System.out.println("=============產生WR01_Operation結束===========");      
%>
...
public String Generate(String Report_no,String s_year,String s_month,String isDebug,String lguser_id) throws Exception{    	
    	...
    	String wr_rpt = "1";//警示報表類別 0:與上月比較 1:與上季比較 2:與上年度同期比較
...
try{      
	  ...
	  if(!logDir.exists()){
	   ...
	   logps.println(Utility.getDateFormat("yyyy/MM/dd  HH:mm:ss  ")+" "+"產生"+s_year+"年"+s_month+"月 (農.漁會-各別機構)WR01_opeation與上季比較 資料中");		    					    
	   logps.flush();
  	...
  	List dbData = getDetailInfo(s_year,s_month,lastSeason_year,lastSeason_month,wlx01_m_year);
    if(isDebug.equals("true"))  System.out.println("***********dbData.size()="+dbData.size());
    if(dbData.size() > 0){
        String[] field = {"field_debit_rate","field_credit_rate","field_990610_rate","field_noassure_rate","field_992150_rate",
						 "field_120700_rate","field_992710","field_992710_rate","field_992710_credit_rate","field_992710_990230_rate",
						 ...
						 "field_992610_cal_credit_rate","field_diff_992630","field_diff_992730","field_320300","field_diff_320300",
						 "field_320300_520800","field_diff_backup","field_backup_over_rate","field_200000_cal_net_rate","field_320100",
						 "field_990230","field_140000_rate","field_diff_captial_rate"};
    	...
    	for(int i=0;i<dbData.size();i++){
	    	...
	    	bank_code = (bean.getValue("bank_no")).toString();
	    	boolean isIns = true;
		...
		if(!DBManager.updateDB_ps(updateDBList)){					 
	  	...
	  	//增加逾放比率.平均數、負債/淨值.同業平均數
	  	List getAVG = getAVG(s_year,s_month,wlx01_m_year);
	  	if(isDebug.equals("true"))  System.out.println("***********getAVG.size()="+getAVG.size());
	  	String field_over_rate_avg = "";
	  	String field_200000_cal_net_rate_avg = "";
	  	if(getAVG.size()>0){
	  		AVGBean = (DataObject) getAVG.get(0);
	  		field_over_rate_avg = (AVGBean.getValue("field_over_rate_avg")).toString();
	  		field_200000_cal_net_rate_avg = (AVGBean.getValue("field_200000_cal_net_rate_avg")).toString();
	  	}
	  	updateDBSqlList = new ArrayList();		
    	updateDBDataList = new ArrayList();
    	updateDBList = new ArrayList();
	  	sqlCmd.delete(0, sqlCmd.length());
		dataList = new ArrayList();//傳內的參數List
		sqlCmd.append("insert into WR_OPERATION_TMP(M_YEAR,M_MONTH,WR_RPT,BANK_CODE,ACC_CODE,TYPE,AMT) values(?,?,?,?,?,?,?)");
		dataList.add(s_year); 
	    dataList.add(s_month); 
	    dataList.add(wr_rpt); 
	    dataList.add("ALL");
	    dataList.add("field_over_rate_avg"); 
	    dataList.add("4"); 
	    dataList.add(field_over_rate_avg);
	    updateDBSqlList.add(sqlCmd.toString());//0:欲執行的sql 
	    updateDBDataList.add(dataList);//1:傳內的參數List
	    ...
	    dataList.add(wr_rpt); 
	    dataList.add("ALL");
	    dataList.add("field_200000_cal_net_rate_avg"); 
	    dataList.add("4"); 
	    dataList.add(field_200000_cal_net_rate_avg);
	    updateDBSqlList.add(sqlCmd.toString());//0:欲執行的sql 
	    updateDBDataList.add(dataList);//1:傳內的參數List
	  	...
	  	for(int r=0;r<rangeInfo.size();r++){
	  		...
	  		if(!"max".equals(rangeBean_quop) && !"min".equals(rangeBean_quop)){
	  		    ...
	  		    if(!("0".equals(tmpStr)||"1".equals(tmpStr)||"2".equals(tmpStr)||"3".equals(tmpStr)||"4".equals(tmpStr)
	  		      ...
	  		      if("field_200000_cal_net_rate".equals(rangeBean_acc_code)){
	  		        	rangeBean_field_amt = field_200000_cal_net_rate_avg;
	  		      }else{
	  		...
	  		if(operTmpRangeInfo.size()>0){
	    	    ...
	    	    for(int k=0;k<size;k++){
	                ...
	                String operTmpRangeBean_warn_type = (operTmpRangeBean.getValue("warn_type")==null)?"":(operTmpRangeBean.getValue("warn_type")).toString();
	                if("N".equals(operTmpRangeBean_warn_type)){
						dataList = new ArrayList();//傳內的參數List
						dataList.add("Y");
		...
		if(!DBManager.updateDB_ps(updateDBList)){	
			...
			logps.println(Utility.getDateFormat("yyyy/MM/dd  HH:mm:ss  ")+" "+"執行 WR01_opeation與上季比較 失敗");
		}
		else{
		    logps.println(Utility.getDateFormat("yyyy/MM/dd  HH:mm:ss  ")+" "+"產生 WR01_opeation與上季比較 完成");
...
private List getDetailInfo(String s_year,String s_month,String lastSeason_year,String lastSeason_month,String wlx01_m_year){
    StringBuffer sqlCmd = new StringBuffer();        	
	...
	sqlCmd.append("        decode(a01_last.field_992150,0,0,round((a01.field_992150 - a01_last.field_992150)/  a01_last.field_992150 *100 ,2))  as field_992150_RATE,");//--增加金額/無擔保消費性放款       
	...
	sqlCmd.append("        decode(a01.field_990230,0,0,round(a01.field_992710 /  a01.field_990230 *100 ,2)) -   decode(a01_last.field_990230,0,0,round(a01_last.field_992710 /  a01_last.field_990230 *100 ,2)) as field_diff_992710_990230_RATE,");//--本月份之(建築放款/上年度信用部決算淨值)-上季之(建築放款/上年度信用部決算淨值)   
	...
	sqlCmd.append("        decode(a01.field_CREDIT,0,0,round(a01.field_OVER /  a01.field_CREDIT *100 ,2)) - decode(a01_last.field_CREDIT,0,0,round(a01_last.field_OVER /  a01_last.field_CREDIT *100 ,2)) as field_diff_OVER_RATE,");//--本月份逾放比率-上季逾放比率       
	...
	sqlCmd.append("        round(a01.field_992610_cal /1,0)  as field_992610_cal,");  //--應予觀察放款
	...
	sqlCmd.append("        round(a01.field_320300 /1,0)  as field_320300,");//--本期損益
	sqlCmd.append("        a01.field_320300 - a01_last.field_320300 as field_diff_320300,");//--本期損益增加金額
	sqlCmd.append("        round(a01.field_320300_520800 /1,0)  as field_320300_520800,");//--本期損益(不含提列呆帳)
	...
	sqlCmd.append("        decode(a01.field_NET,0,0,round(a01.field_200000_cal /  a01.field_NET *100 ,2))  as   field_200000_cal_NET_RATE,");//--負債/淨值
	sqlCmd.append("        round(a01.field_320100 /1,0)  as field_320100,");//--累積虧損
	sqlCmd.append("        round(a01.field_990230 /3,0)  as field_990230,");//--上年度信用部決算後淨值*1/3
	sqlCmd.append("        decode(a01_last.field_140000,0,0,round((a01.field_140000 - a01_last.field_140000)/  a01_last.field_140000 *100 ,2))  as   field_140000_RATE,");//--增減金額/固定資產淨額%
	sqlCmd.append("        round((a01.field_CAPTIAL - a01_last.field_CAPTIAL) / 1000 ,2)  as   field_diff_CAPTIAL_RATE  ");//--本月份BIS -上季BIS
	...
	sqlCmd.append("          SUM(field_320300_520800)  as field_320300_520800,");
	sqlCmd.append("          SUM(field_NET)    as field_NET,");
	sqlCmd.append("          SUM(field_OVER)   as field_OVER,");
	sqlCmd.append("          SUM(field_BACKUP) as field_BACKUP,");
	sqlCmd.append("          SUM(field_200000_cal) as field_200000_cal,");
	sqlCmd.append("          SUM(field_320100) as field_320100,");
	...
	sqlCmd.append("          SUM(field_CAPTIAL) as field_CAPTIAL,");
	sqlCmd.append("          SUM(field_990230) as field_990230,");
	sqlCmd.append("          SUM(field_990320) as field_990320 ");
	sqlCmd.append("        from ");
	...
	sqlCmd.append("            round(sum(decode(a01.acc_code,'320300',amt,'520800',-amt,0)) /1,0) as field_320300_520800,");
	...
	sqlCmd.append("            round(sum(decode(bank_type,'6',decode(a01.acc_code,'210000',amt,'220000',amt,'240000',amt,'250000',amt,'260000',amt,0),");
	sqlCmd.append("                                       '7',decode(a01.acc_code,'200000',amt,0),0))/1,0) as field_200000_cal,");
	sqlCmd.append("            round(sum(decode(a01.acc_code,'320100',amt,0)) /1,0) as field_320100,");
	...
	sqlCmd.append("           round(sum(decode(a02.acc_code,'990320',amt,0)) /1,0) as field_990320 ");
	sqlCmd.append("         from (select * from bn01 where m_year = ? and bank_type in ('6','7') and bn_type <> '2')bn01 ");
	paramList.add(wlx01_m_year);
	sqlCmd.append("         left join (select * from a02 where m_year = ? and m_month=? and a02.ACC_code in ('990610','990230','990320') ) a02 on  bn01.bank_no = a02.bank_code ");
	paramList.add(s_year);
	paramList.add(s_month);
	...
	sqlCmd.append("        ) a99,");
	sqlCmd.append("        (select a05.m_year,a05.m_month,bn01.bank_no as bank_code,  bn01.bank_name,");
	...
	paramList.add(s_year);
	paramList.add(s_month);
	sqlCmd.append("        group by a05.m_year,a05.m_month,bn01.bank_no,bn01.BANK_NAME ");
	sqlCmd.append("        ) a05 ");
	sqlCmd.append("        where a01.bank_no=a02.bank_code(+) and a01.bank_no = a99.bank_code(+) and a01.bank_no=a05.bank_code(+) ");
	sqlCmd.append("        and a01.bank_no <> ' ' ");
	sqlCmd.append("        GROUP BY a01.m_year,a01.m_month,a01.bank_no,a01.BANK_NAME ");
	sqlCmd.append(" ) a01,");//--本季資料
	sqlCmd.append(" ( select a01.m_year,a01.m_month, a01.bank_no, a01.BANK_NAME,");
	...
	sqlCmd.append("          SUM(field_992150) field_992150,");
	sqlCmd.append("          SUM(field_992720) field_992720,");
	sqlCmd.append("          SUM(field_992630) field_992630,");
	sqlCmd.append("          SUM(field_992730) field_992730,");
	sqlCmd.append("          SUM(field_CAPTIAL) field_CAPTIAL,");
	sqlCmd.append("          SUM(field_990230) field_990230,");
	sqlCmd.append("          SUM(field_990320) field_990320 ");
	sqlCmd.append("        from ");
	sqlCmd.append("        ( select a01.m_year,a01.m_month, bn01.bank_no , bn01.BANK_NAME,");
	sqlCmd.append("            round(sum(decode(a01.acc_code,'220000',amt,0)) /1,0) as field_DEBIT,");
	sqlCmd.append("            round(sum(decode(a01.acc_code,'120000',amt,'120800',amt,'150300',amt,0)) /1,0) as  field_CREDIT,");
	...
	sqlCmd.append("                                   ELSE '00' END) as YEAR_TYPE,m_year,m_month,bank_code,acc_code,amt from a01 ");
	sqlCmd.append("                      where m_year=? and m_month=? ");
	paramList.add(lastSeason_year);
	paramList.add(lastSeason_month);
	sqlCmd.append("                      ) a01  on  bn01.bank_no = a01.bank_code ");
	sqlCmd.append("          group by a01.m_year,a01.m_month,bn01.bank_no,bn01.BANK_NAME ");
	sqlCmd.append("        ) a01,");
	sqlCmd.append("        (select a02.m_year,a02.m_month, bn01.bank_no as bank_code,  bn01.bank_name,");
	...
	sqlCmd.append("        and a01.bank_no <> ' ' ");
	sqlCmd.append("        GROUP BY a01.m_year,a01.m_month,a01.bank_no,a01.BANK_NAME ");
	sqlCmd.append(" ) a01_last ");//--上季資料
	sqlCmd.append(" where a01.bank_no=a01_last.bank_no(+) ");
	List dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(),paramList,"m_year,m_month,bank_no,bank_name,field_debit_rate,field_credit_rate,field_990610_rate,field_noassure_rate,"
																		+"field_992150_rate,field_120700_rate,field_992710,field_992710_rate,field_992710_credit_rate,field_992710_990230_rate,"
																		+"field_diff_992710_990230_rate,field_over,field_diff_over,field_over_rate,field_diff_over_rate,field_diff_992530,"
																		+"field_992550_992150_rate,field_992720_992710_rate,field_992610_cal,field_diff_992610_cal,field_992610_cal_credit_rate,"
																		+"field_diff_992630,field_diff_992730,field_320300,field_diff_320300,field_320300_520800,field_diff_backup,"
																		+"field_backup_over_rate,field_200000_cal_net_rate,field_320100,field_990230,field_140000_rate,field_diff_captial_rate");
	return dbData;
}
...
private List getOperTmpRangeInfo(String m_year,String m_month,String acc_code,String quop,String field_amt,String wr_rpt){
	...
	if(!"max".equals(quop) && !"min".equals(quop)){
	    if("field_320100".equals(acc_code)){ //取絕對值
	        sqlCmd.append("  and abs(amt) ").append(quop).append(" ").append(field_amt);
	    }else{
...
private List getAVG(String m_year,String m_month,String wlx01_m_year){
    StringBuffer sqlCmd = new StringBuffer();        	
	List paramList = new ArrayList();
	sqlCmd.append(" select decode(a01.field_CREDIT,0,0,round(a01.field_OVER /  a01.field_CREDIT *100 ,2)) as field_over_rate_avg,");//--逾放比率.平均數
	sqlCmd.append("        decode(a01.field_NET,0,0,round(a01.field_200000_cal /  a01.field_NET *100 ,2)) as field_200000_cal_net_rate_avg ");//--負債/淨值.同業平均數
	sqlCmd.append("   from (select SUM(field_CREDIT) as field_CREDIT,");
	sqlCmd.append("                SUM(field_NET)    as field_NET,");
	sqlCmd.append("                SUM(field_OVER)   as field_OVER,"); 
	sqlCmd.append("                SUM(field_200000_cal) as field_200000_cal ");
	sqlCmd.append("           from ");
	...
	sqlCmd.append("                       round(sum(decode(bn01.bank_type,'6',decode(a01.acc_code,'310000',amt,'320000',amt,0),'7',decode(a01.acc_code,'300000',amt,0),0)) /1,0) as field_NET,");
	sqlCmd.append("                       round(sum(decode(a01.acc_code,'990000',amt,0)) /1,0) as field_OVER,");
	sqlCmd.append("                       round(sum(decode(bank_type,'6',decode(a01.acc_code,'210000',amt,'220000',amt,'240000',amt,'250000',amt,'260000',amt,0),'7',decode(a01.acc_code,'200000',amt,0),0))/1,0) as field_200000_cal ");
	sqlCmd.append("                  from (select * from bn01 where m_year = ? and bank_type in ('6','7') and bn_type <> '2')bn01 ");
	paramList.add(wlx01_m_year);
	sqlCmd.append("                     left join (select (CASE WHEN (a01.m_year <= 102) THEN '102' WHEN (a01.m_year > 102) THEN '103' ELSE '00' END) as YEAR_TYPE,m_year,m_month,bank_code,acc_code,amt from a01 ");
	sqlCmd.append("                                 where m_year=? and m_month=? ");
	paramList.add(m_year);
	paramList.add(m_month);
	sqlCmd.append("                                ) a01  on  bn01.bank_no = a01.bank_code ");
	...
	List dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(),paramList,"field_over_rate_avg,field_200000_cal_net_rate_avg");
	return dbData;
}
