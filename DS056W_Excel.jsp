...
<%
	...
	try{
			...
			sqlCmd.append("        		field_loan_bal_cnt_last,");
			sqlCmd.append("        		field_loan_bal_amt_last,");
			sqlCmd.append("        		field_loan_bal_amt - field_loan_bal_amt_last as field_loan_bal_amt_diff,");
			sqlCmd.append("        		decode(field_loan_bal_amt_last,0,0,round((field_loan_bal_amt-field_loan_bal_amt_last) / field_loan_bal_amt_last *100 ,2))  as   field_loan_bal_amt_rate,");//--貸放餘額增加幅度%(A-B)/B
			sqlCmd.append("        		'0' as type_loan_bal_cnt,");
			sqlCmd.append("        		'0' as type_loan_bal_amt,");
			sqlCmd.append("        		'0' as type_loan_bal_cnt_last,");
			sqlCmd.append("        		'0' as type_loan_bal_amt_last,");
			sqlCmd.append("        		'0' as type_loan_bal_amt_diff,");
			sqlCmd.append("        		'4' as type_loan_bal_amt_rate ");
			sqlCmd.append(" 	   from (select agri_loan.m_year,agri_loan.m_month, agri_loan.bank_code ,");
			sqlCmd.append("               		decode(SUM(agri_loan.field_loan_bal_cnt),null,0,SUM(agri_loan.field_loan_bal_cnt)) as field_loan_bal_cnt,");//--當月-(6)貸放餘額-戶數
			sqlCmd.append("               		decode(SUM(agri_loan.field_loan_bal_amt),null,0,SUM(agri_loan.field_loan_bal_amt)) as field_loan_bal_amt,");//--當月-(7)貸放餘額
			sqlCmd.append("               		decode(SUM(agri_loan_last.field_loan_bal_cnt),null,0,SUM(agri_loan_last.field_loan_bal_cnt)) as field_loan_bal_cnt_last,");//--上月-(6)貸放餘額-戶數
			sqlCmd.append("               		decode(SUM(agri_loan_last.field_loan_bal_amt),null,0,SUM(agri_loan_last.field_loan_bal_amt)) as field_loan_bal_amt_last ");//--上月-(7)貸放餘額
			sqlCmd.append("        		   from ");
            ...
            for(i=0;i<dbData.size();i++){            
                ...
                for(int cellIdx =4;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                     ...
                     if(!(amt_type.equals("4") || "loan_bal_cnt".equals(tmpStr) || "loan_bal_cnt_last".equals(tmpStr))){//該值不為利率或戶數時.再除以單位4捨五入
