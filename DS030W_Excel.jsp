<%
//100.06.29 create 自有資本與風險性資產比率計算表(R0521) by 2295
...
<%
	...
	try{	      	
			...
			titleName += "自有資本與風險性資產比率計算表(R0521)";	  		
			//年
            ...
            */                                   
			...
			if(dbData == null || dbData.size() == 0){
                ...
                for(i=0;i<dbData.size();i++){            
                    ...
                    for(int cellIdx =2;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                         ...
                         if(cell.getStringCellValue().trim().startsWith("99")){
			...
			//100.07.05 合併acc_code時,科目代號跟欄位名稱時,合併後的列會縮小;不合併		
			/*	
            for(i=0;i<btnFieldList_data.size();i++){
