...
<%
    ...
    String SortList = "";
    String CANCEL_NO = "";
    String S_YEAR = "";//99.12.27 add
    String S_MONTH = "";//99.12.27 add
	...
	try{
			...
			p.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"WLX02_M.length"));
			//====================================================================================================
            ...
            String column = "";//選取欄位            
            String condition = " WLX02_M.bank_no = BN02.bank_no "
				      		 + " and BN02.tbank_no = BN01.bank_no";
            ...
            if(CANCEL_NO.equals("N")){//營運中
			   condition += " and BN02.bn_type <> '2'";//條件
			}else{//已裁撤
			   condition += " and BN02.bn_type = '2'";//條件
			}			  	 
			//======================================================	      		 					  		 					  		 
            String selectBank_no = "";//選取的金融機構代號
            String table = " (select * from bn02 where m_year=?)BN02,(select * from bn01 where m_year=?)BN01,WLX02_M";//查詢table
            String order = "";//排序欄位
            String sqlCmd="";
            ...
            for(i=0;i<btnFieldList_data.size();i++){
               ...
               System.out.println("columnLen="+columnLen[i]);
               //選取欄位         			 
			   if(((String)((List)btnFieldList_data.get(i)).get(0)).equals("BN01.BANK_NAME")){
			       column += " BN01.BANK_NAME as tbank_no_name";			   	  			   
			   }else if(((String)((List)btnFieldList_data.get(i)).get(0)).equals("WLX02_M.POSITION_CODE")){
			       column += " WLX02_M.POSITION_CODE,cdshareno.cmuse_name as position_code_name";			   	  
			   }else{
               ...
               //條件式跟table=============================================================================
               if(((String)((List)btnFieldList_data.get(i)).get(0)).equals("WLX02_M.POSITION_CODE")){               	  
               	  table += " LEFT JOIN cdshareno on WLX02_M.position_code = cdshareno.cmuse_id and cdshareno.cmuse_div='007'";
               }
               //=========================================================================================
            }            
            ...
            if(!position_code.equals("ALL")){
               ...
               condition += " and WLX02_M.position_code = ? ";
               //condition += " and WLX02_M.position_code = '"+position_code+"' ";
            }
            //排序欄位=========================================================================
            ...
            /*  SELECT BN01.BANK_NO as tbank_no,BN01.BANK_NAME as tbank_no_name,BN02.BANK_NO,
	   				   BN02.BANK_NAME,WLX02_M.NAME,WLX02_M.SEX,
	   				   WLX02_M.POSITION_CODE,cdshareno.cmuse_name as position_code_name,
	   				   WLX02_M.RANK,WLX02_M.ID,
	   				   WLX02_M.BIRTH_DATE,WLX02_M.DEGREE,WLX02_M.TELNO,
	   				   WLX02_M.INDUCT_DATE,WLX02_M.BACKGROUND
				FROM BN02,BN01,
					 WLX02_M LEFT JOIN cdshareno on WLX02_M.position_code = cdshareno.cmuse_id and cdshareno.cmuse_div='007'	 
				WHERE WLX02_M.bank_no = BN02.bank_no 
				      and BN02.tbank_no = BN01.bank_no
					  and BN02.tbank_no IN ('6030016')
					  and BN02.bn_type <> '2'   
  			*/
  			...
  			System.out.println("BR006W_Excel.sqlCmd="+sqlCmd);	   
			...
			if(dbData == null || dbData.size() == 0){
                ...
                reportUtil.createCell( wb, row, ( short )1,"無分支機構高階主管基本資料" ,noBorderDefaultStyle ); 
            ...
            for ( i = 0; i < dbData.size(); i ++) {                
                ...
                for(int j=0;j<btnFieldList_data.size();j++){
			        ...
			        if(field.equals("birth_date") || field.equals("induct_date")){//日期型態處理
			           ...
			           if((field).equals("bn02.bank_name")){field = "bank_name";}			           			           		           			           
			           ...
			           if(field.equals("rank")){//順位
			              ...
			              if(field.equals("tbank_no")){//總機構代號
			                 //94.02.17 fix 每一筆bank_no,bank_name都要顯示
			                 reportUtil.createCell( wb, row, ( short )(j+1),(String)((DataObject)dbData.get(i)).getValue(field) ,defaultStyle );					
			                /*只顯示第一筆bank_no,相同則不顯示
			                if(!((String)((DataObject)dbData.get(i)).getValue(field)).equals(tmptbank_no)){ 
			                   reportUtil.createCell( wb, row, ( short )(j+1),(String)((DataObject)dbData.get(i)).getValue(field) ,defaultStyle );					
			                   tmptbank_no = (String)((DataObject)dbData.get(i)).getValue(field);
			                }else{
			                   reportUtil.createCell( wb, row, ( short )(j+1),"",defaultStyle );					
			                }*/
			              }else if(field.equals("tbank_no_name")){//總機構名稱   
			                 reportUtil.createCell( wb, row, ( short )(j+1),(String)((DataObject)dbData.get(i)).getValue(field) ,defaultStyle );					
			                 /*只顯示第一筆bank_name,相同則不顯示
			                 if(!((String)((DataObject)dbData.get(i)).getValue(field)).equals(tmptbank_name)){ 
			                   reportUtil.createCell( wb, row, ( short )(j+1),(String)((DataObject)dbData.get(i)).getValue(field) ,defaultStyle );					
			                   tmptbank_name = (String)((DataObject)dbData.get(i)).getValue(field);
			                 }else{
			                   reportUtil.createCell( wb, row, ( short )(j+1),"",defaultStyle );					
			                 }*/
			                 /*
			              }else if(field.equals("bank_no")){//分支機構代號 
			              ...
			              }else if(field.equals("bank_name")){//分支機構名稱   
