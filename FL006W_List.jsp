...
<%
    ...
    String bank_Rt_DocNo = Utility.getTrimString(dataMap.get("bank_Rt_DocNo")) ; 
...
<tr class="sbody" >
       ...
       <td bgcolor="#EBF4E1" height="1">
       <input type="text" name="bank_Rt_DocNo" value='<%=bank_Rt_DocNo %>' size="49" maxlength='10'>
</tr>  
</Table>
...
<% if(!"".equals(ex_Type)) {%>
    ...
    <tr class="sbody" bgcolor="#BFDFAE">
    	<td width="8%" height="14" rowspan='2'>機構代號</td>
    	<td width="20%" height="14" rowspan='2'>機構名稱</td>
      	<td width="12%" height="14" rowspan='2'><%if("FEB".equals(ex_Type)){%>檢查報告編號<%}else if("AGRI".equals(ex_Type)){%>查核季別<%}else{%>訪查日期<%} %></td>
        <td width="12%" height="14" rowspan='2'>收文日期</td>        
      	<td width="12%" height="14" rowspan='2'>收文文號</td>
      	<td height="14" colspan='2' align='center'>辦理依據</td>
    </tr>
    <tr class="sbody" bgcolor="#BFDFAE">
    	
      	<td height="14">發文日期</td>
      	<td height="14">發文文號</td>
    </tr>
    <%
    if(rs != null && rs.size() > 0) {
      ...
      for(int i=0; i<rs.size(); i++) {
        ...
        String doc_type = bean.getValue("doc_type") == null ? "" : (String)bean.getValue("doc_type");
        String nextPage = "Edit";
    %>      
    <tr class="sbody" bgcolor='<%=(i % 2 == 0)?"#EBF4E1":"#FFFFCC"%>'>
      <td width="15%" height="1"><%=bean.getValue("bank_no") != null ? bean.getValue("bank_no") : "&nbsp;"%></td>
      <td width="25%" height="1"><%=bean.getValue("bank_name") != null ? bean.getValue("bank_name") : "&nbsp;"%></td>
      <td width="15%" height="1"><%=bean.getValue("ex_no_list") != null ? bean.getValue("ex_no_list") : "&nbsp;"%></td>
      <td width="15%" height="1"><%=bean.getValue("bank_rt_doc_date") != null ? (String)bean.getValue("bank_rt_doc_date") : "&nbsp;"%></td>
      <td width="15%" height="1">
	      <a href=FL006W.jsp?act=<%=nextPage %>&bank_No=<%=bean.getValue("bank_no")%>&ex_Type=<%=bean.getValue("ex_type")%>&ex_No=<%=bean.getValue("ex_no")%>&bank_Rt_DocNo=<%=bean.getValue("bank_rt_docno")%>>
	      	<%=bean.getValue("bank_rt_docno") != null ? bean.getValue("bank_rt_docno") : "&nbsp;"%>
	      </a>&nbsp;</td>
      <td width="15%" height="1"><%=bean.getValue("doc_date") != null ? (String)bean.getValue("doc_date") : "&nbsp;"%></td>
      <td width="15%" height="1"><%=bean.getValue("docno") != null ? bean.getValue("docno") : "&nbsp;"%></td>
    </tr>
    <%        
      }
    }else{%>            
