...
<%
	...
	try{
			...
			sqlCmd.append("        		field_delay_loan_cnt_sum,");
			sqlCmd.append("        		field_loan_bal_cnt,");
			sqlCmd.append("        		decode(field_loan_bal_cnt,0,0,round(field_delay_loan_cnt_sum/ field_loan_bal_cnt *100 ,2))  as   field_loan_bal_cnt_rate,");
			sqlCmd.append("        		'0' as type_delay_loan_cnt,");
			sqlCmd.append("        		'0' as type_delay_loan_cnt_sum,");
			...
			sqlCmd.append("               		decode(SUM(agri_loan_sum.field_delay_loan_cnt),null,0,SUM(agri_loan_sum.field_delay_loan_cnt)) as field_delay_loan_cnt_sum,");
			sqlCmd.append("               		decode(SUM(agri_loan.field_loan_bal_cnt),null,0,SUM(agri_loan.field_loan_bal_cnt)) as field_loan_bal_cnt ");
			sqlCmd.append("        		   from ");
			...
			sqlCmd.append("         			) agri_loan_sum ");//--當年度
			sqlCmd.append("        		  where agri_loan.bank_code=agri_loan_sum.bank_code(+) ");
            ...
            for(i=0;i<dbData.size();i++){            
                ...
                for(int cellIdx =4;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                     ...
                     if(!(amt_type.equals("4") || "delay_loan_cnt".equals(tmpStr) || "delay_loan_cnt_sum".equals(tmpStr) 
                             || "loan_bal_cnt".equals(tmpStr) || "loan_bal_cnt_last".equals(tmpStr))){//該值不為利率或戶數時.再除以單位4捨五入
