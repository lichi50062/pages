...
//96.01.15 add 金融機構類別/縣市別.(總機構單位+受檢單位)可個別顯示 by 2295
%>
...
<%
    ...
    String bankno_flag = ( request.getParameter("bankno_flag")==null ) ? "" : (String)request.getParameter("bankno_flag");					
...
<td width="<%=tc_value_width%>" bgcolor="#EBF4E1" height="1">
   <select size="1" name="bankType" onChange="checkCity();resetOption_bankno();changeHsienID('HsienIDXML',form.cityType,form.bankType,form);changeCity_bankno('TBankXML','BankNoXML', form.tbank, form.cityType, form)">        											  
  ...
  縣市別:&nbsp;&nbsp; 
  <select size="1" name="cityType" onChange="changeCity_bankno('TBankXML','BankNoXML', form.tbank, form.cityType, form)<%if(showTBank.equals("false")) out.print(";changeOption('BankNoXML',form.examine, form.tbank, 'TBankXML');");%>" >
...
<table border=1 width='100%' align=center bgcolor="#FFFFF" bordercolor="#76C657">
<tr class="sbody">
  <td width="<%=tc_title_width%>" bgcolor="#BDDE9C" height="1"><%if(bankno_flag.equals("1")) out.print("金融機構"); else out.print("受檢單位");%></td>
  <td width="<%=tc_value_width%>" bgcolor="#EBF4E1" height="1">           
