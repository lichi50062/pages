...
<%
	System.out.println("FR030W_Qry.jsp Program Start...");
	...
	if(session.getAttribute("muser_id") == null){
 	...
 	//取得FR030W_Qry的權限
	Properties permission = ( session.getAttribute("FR030W_Qry")==null ) ? new Properties() : (Properties)session.getAttribute("FR030W_Qry");
	
	if(permission == null){
       System.out.println("FR030W_Qry.permission == null");
    }else{
       System.out.println("FR030W_Qry.permission.size ="+permission.size());
    }
...
function doSubmit(){
   if(!confirm("本項報表會執行3-5秒，是否確定執行？"))
   		return;
   this.document.forms[0].action = "/pages/FR030W_Excel.jsp";
        ...
        <tr>
          <td><table width="600" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td width="380"><font color='#000000' size=4>
                  <center>
                  	<% if(bank_type.equals("6")){//農會type為6
                  		%>
                  		<font color="#336600">台灣區農會信用部支票存款戶數與餘額彙計表</font>
                  	<%}else{%>
                  		<font color="#336600">台灣區漁會信用部支票存款戶數與餘額彙計表</font>
                  	<%}%>
                  </center>
                  </font> </td>
                <td width="110"><img src="images/banner_bg1.gif" width="110" height="17"></td>
                          ...
                          <td><img src="images/2_icon_01.gif" width="16" height="16" align="absmiddle"><span class="mtext">查詢年月 :</span>
                            <input type='text' name='S_YEAR' value="<%=YEAR%>" size='3' maxlength='3' onblur='CheckYear(this)'><font color='#000000'>年
        						...
        						<%
                                                        for (int j = 1; j <= 12; j++) {
                                                        	if (j < 10){%>
        								<option value=<%=j%> <%if(MONTH.equals(String.valueOf(j))) out.print("selected");%>>0<%=j%></option>
            							<%}else{%>
...
<% System.out.println("FR030W_Qry.jsp Program End...");%>
</body>
</html>