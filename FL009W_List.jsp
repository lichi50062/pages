...
<%
    ...
    String corr_DocNo = Utility.getTrimString(dataMap.get("corr_DocNo")) ; 
...
<tr class="sbody" >
       ...
       <td bgcolor="#EBF4E1" height="1">
       <input type="text" name="corr_DocNo" value='<%=corr_DocNo %>' size="49" maxlength='10'>
</tr>  
</Table>
...
<% if(!"".equals(ex_Type)) {%>
    ...
    <tr class="sbody" bgcolor="#BFDFAE">
    	<td width="8%" rowspan='2'>機構代號</td>
    	<td width="25%" rowspan='2'>機構名稱</td>
      	<td width="11%" rowspan='2'><%if("FEB".equals(ex_Type)){%>檢查報告編號<%}else if("AGRI".equals(ex_Type)){%>查核季別<%}else{%>訪查日期<%} %></td>
        <td width="11%" rowspan='2'>收文日期</td>        
    ...
    <tr class="sbody" bgcolor='<%=(i % 2 == 0)?"#EBF4E1":"#FFFFCC"%>'>
      ...
      <td><%=bean.getValue("corr_doc_date") != null ? (String)bean.getValue("corr_doc_date") : "&nbsp;"%></td>
      <td>
	      <a href=FL009W.jsp?act=Edit&bank_No=<%=bean.getValue("bank_no")%>&ex_Type=<%=bean.getValue("ex_type")%>&ex_No=<%=bean.getValue("ex_no")%>&docNo=<%=(String)bean.getValue("docno")%>&corr_DocNo=<%=bean.getValue("corr_docno")%>>
	      	<%=bean.getValue("corr_docno") != null ? bean.getValue("corr_docno") : "&nbsp;"%>
	      </a>&nbsp;</td>
