...
<%@ page import="com.tradevan.util.report.RptFR002W" %>
...
<%
   ...
   if(session.getAttribute("muser_id") == null){//session timeout		
      System.out.println("FR002W login timeout");   
   		...
   		if(!CheckPermission(request)){//無權限時,導向到LoginError.jsp
	    	...
	    	if(act.equals("Qry")){
   			...
   			if(act.equals("view") || act.equals("download")){
   				try{		    
	    			actMsg = RptFR002W.createRpt(S_YEAR,S_MONTH,Unit,datestate,bank_type);	    
...
<%!
    ...
    private final static String QryPgName = "/pages/FR002W_Qry.jsp";        
    private final static String RptCreatePgName = "/pages/FR002W_Excel.jsp";        
    private final static String LoginErrorPgName = "/pages/LoginError.jsp";
    private boolean CheckPermission(HttpServletRequest request){//檢核權限    	        		
    	    boolean CheckOK=false;
    	    HttpSession session = request.getSession();            
            Properties permission = ( session.getAttribute("FR002W")==null ) ? new Properties() : (Properties)session.getAttribute("FR002W");				                
            if(permission == null){
              System.out.println("FR002W.permission == null");
            }else{
               System.out.println("FR002W.permission.size ="+permission.size());
