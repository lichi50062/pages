...
if(session.getAttribute("btnFieldList") != null && !((String)session.getAttribute("btnFieldList")).equals("")){ 
   System.out.println("DS069W_RptOrder.btnFieldList="+(String)session.getAttribute("btnFieldList"));
%>
<%}%>
getbtnFieldList();
function getbtnFieldList(){
	...
	if(this.document.forms[0].BankList.value.indexOf("ALL") == -1){
       ...
       for(var k =0;k<this.document.forms[0].SortListDst.length;k++){			
	       if(this.document.forms[0].SortListDst.options[k].text == "查詢年月"){		    
		      checkAdd = true;			       
	       }  
	   }  
	   if(checkAdd == false){	  
	      this.document.forms[0].SortListSrc.options[addCount] = new Option("查詢年月", "a15.m_year,a15.m_month");
	      addCount++;
	   }
	   checkAdd = false;
	   for(var k =0;k<this.document.forms[0].SortListDst.length;k++){			
       ...
       if(checkAdd == false){	  
          this.document.forms[0].SortListSrc.options[addCount] = new Option("金融機構代號名稱", "a15.bank_code");
          addCount++;
       }
    }else{//已選的機構代號=ALL全部
       /*95.11.16 拿掉年 sort
