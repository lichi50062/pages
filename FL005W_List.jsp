...
<% if(!"".equals(ex_Type)) {%>
    ...
    <tr class="sbody" bgcolor="#BFDFAE">
    	<td width="15%" height="14">機構代號</td>
    	<td width="25%" height="14">機構名稱</td>
      	<td width="15%" height="14"><%if("FEB".equals(ex_Type)){%>檢查報告編號<%}else if("AGRI".equals(ex_Type)){%>查核季別<%}else{%>訪查日期<%} %></td>
        <td width="15%" height="14">發文日期</td>        
      	<td width="15%" height="14">發文文號</td>
      	<td width="15%" height="14">發文性質</td>
    </tr>
    <%
    if(rs != null && rs.size() > 0) {
      ...
      for(int i=0; i<rs.size(); i++) {
        ...
        String nextPage = "DetailList";
        if(!"B".equals(doc_type)){
        	nextPage = "Edit";
        }
    %>      
    <tr class="sbody" bgcolor='<%=(i % 2 == 0)?"#EBF4E1":"#FFFFCC"%>'>
      ...
      <td width="15%" height="1">
	      <a href=FL005W.jsp?act=<%=nextPage %>&bank_No=<%=bean.getValue("bank_no")%>&ex_Type=<%=bean.getValue("ex_type")%>&ex_No=<%=bean.getValue("ex_no")%>&docNo=<%=bean.getValue("docno")%>>
      ...
      <td width="15%" height="1"><%=bean.getValue("doc_type_name") != null ? bean.getValue("doc_type_name") : "&nbsp;"%></td>
    </tr>
    <%        
      }
    }else{%>            
