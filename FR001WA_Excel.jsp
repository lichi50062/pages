...
<%@ page import="com.tradevan.util.report.RptFR001WA" %>								          
...
<%
   ...
   String printStyle = ( request.getParameter("printStyle")==null ) ? "" : (String)request.getParameter("printStyle"); //108.05.03 add    
   ...
   if(session.getAttribute("muser_id") == null){//session timeout		
      System.out.println("FR001WA login timeout");   
   		...
   		if(!CheckPermission(request)){//無權限時,導向到LoginError.jsp
	    	...
	    	if(act.equals("Qry")){
   			...
   			if(act.equals("view") || act.equals("download")){
   				try{		    
	    			actMsg = RptFR001WA.createRpt(S_YEAR,S_MONTH,Unit,datestate,bank_type,rptStyle);	    
...
<%!
    ...
    private final static String QryPgName = "/pages/FR001WA_Qry.jsp";        
    private final static String RptCreatePgName = "/pages/FR001WA_Excel.jsp";        
    private final static String LoginErrorPgName = "/pages/LoginError.jsp";
    private boolean CheckPermission(HttpServletRequest request){//檢核權限    	        		
    	    boolean CheckOK=false;
    	    HttpSession session = request.getSession();            
            Properties permission = ( session.getAttribute("FR001WA")==null ) ? new Properties() : (Properties)session.getAttribute("FR001WA");				                
            if(permission == null){
