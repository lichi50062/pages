	...
	try{	      	
			...
			main_table.append("  select m_year,m_month,bank_code"+field_sum);			
 			main_table.append("  from ( select m_year,m_month,bank_code,acc_code,amt from agribank_rpt ");
  			//main_table.append("          where (to_char(m_year * 100 + m_month) >= ? and to_char(m_year * 100 + m_month) <= ?)");		
  			main_table.append("          where ((m_year * 100 + m_month) >= ? and (m_year * 100 + m_month) <= ?)");//101.11.05					   
