function doSubmit(form,cnd){
	    form.action="/pages/TC35.jsp?act="+cnd+"&test=nothing";
...
//重導TC35.jsp頁面以致輸出對應之金融機構資料
function getData(form,cnd,item){
	if(item == 'bank_type'){
		//初始BANK_NO值
	   	form.BANK_NO.value="";
	}
	form.action="/pages/TC35.jsp?act=getData&nowact="+cnd+"&test=nothing";
	form.submit();
}
...
function checkData(form)
{
	...
	//	return false;
	//}
	if (trimString(form.DELAY_DAYS.value) != "" ){
        if(isNaN(Math.abs(form.DELAY_DAYS.value))){
           alert("逾期天數不可為文字");
           form.DELAY_DAYS.focus();
           return false;
        }
	}
   return true;
