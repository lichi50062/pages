function doSubmit(form,cnd,tbank,violate_date){
	if(cnd =='add'){	   
       ...
       var seltxt=['發文對象'];
	   var input=[form.dffeNum,form.dffeTitle,form.dffeContent];//text textarea陣列
       var inputtxt=['發文字號','函令主旨','內容'];
       if (!chkSelect(sel,seltxt)) return;     //檢核必選select是否皆已選
       else if (!chkInTxt(input,inputtxt)) return;
       if(AskInsert_Permission())
	   {
		 form.action="/pages/MC011W.jsp?act=Insert";	    
		 form.submit();
       }
    }
    else if(cnd =='modify'){	     
	     if(AskEdit_Permission()){
	        form.action="/pages/MC011W.jsp?act=Update";	    
	        form.submit();
	     }
   }
   else if(cnd =='delete'){
	       if(AskDelete_Permission()){
	          form.action="/pages/MC011W.jsp?act=Delete";	    
	          form.submit();
	      }
   }
   else if(cnd == "Qry") {
...
{
	if(form.begY.value != "") {
        form.dffeDate.value = mergeDate(form.begY.value, form.begM.value, form.begD.value);
        //alert(form.VIOLATE_DATE.value);
        if(!fnValidDate(form.dffeDate.value)) {
            alert("發文日期不合法");
            return false;
        }
   }
