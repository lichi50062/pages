...
{
	if(form.report_type.value=='1' || form.report_type.value=='2')
	{
...
function fn_ShowPanel(form,cnd){
	...
	this.document.forms[0].action = "/pages/FR036WW.jsp?act="+cnd+"&report_type="+form.report_type.value;	  
	//this.document.forms[0].action = "/pages/FR036WW.jsp?act="+cnd;	
	//if(cnd == 'createRpt'){
...
function checkflag() { 
  if(document.getElementById("rptStyle").value == "0" ) {
    document.getElementById("HSIEN_ID").disabled = true; 
  ...
  if(document.getElementById("rptStyle").value == "1" ) {
    document.getElementById("HSIEN_ID").disabled = false;
...
function removeBankOption() {
	var year = this.document.forms[0].S_YEAR.value==''? 99 : eval(this.document.forms[0].S_YEAR.value) ;
	syear = '' ;
	if(year > 99) {
		syear = '100' ;
	}else {
    ...
    for (i=target.options.length-1; i>=0; i--){   //倒過來移除 options
       if (target.attr('year')!=syear)
    	   target.remove(i);
    }
