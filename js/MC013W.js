function doSubmit(form,cnd,tbank,violate_date){
	if(cnd =='add'){	   
       ...
       var seltxt=['受處分機構'];
	   var input=[form.apName,form.apTitle,form.apContent,form.apLaw,form.apPs];//text textarea陣列
       var inputtxt=['受處分人','主旨','事實及理由','法令依據','注意事項'];
       if (!chkSelect(sel,seltxt)) return;     //檢核必選select是否皆已選
       else if (!chkInTxt(input,inputtxt)) return;
       if(AskInsert_Permission())
	   {
		 form.action="/pages/MC013W.jsp?act=Insert";	    
		 form.submit();
       }
    }
    else if(cnd =='modify'){	     
	     if(AskEdit_Permission()){
	        form.action="/pages/MC013W.jsp?act=Update";	    
	        form.submit();
	     }
   }
   else if(cnd =='delete'){
	       if(AskDelete_Permission()){
	          form.action="/pages/MC013W.jsp?act=Delete";	    
	          form.submit();
	      }
   }
   else if(cnd == "Qry") {
...
{
	if(form.begY.value != "") {
        form.apDate.value = mergeDate(form.begY.value, form.begM.value, form.begD.value);
        //alert(form.VIOLATE_DATE.value);
        if(!fnValidDate(form.apDate.value)) {
            alert("受處分日期不合法");
            return false;
        }
   }
