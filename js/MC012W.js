function doSubmit(form,cnd,tbank,violate_date){
	if(cnd =='add'){	   
       ...
       var seltxt=['舞弊機構'];
	   var input=[form.emNum,form.emEmer,form.emContent];//text textarea陣列
       var inputtxt=['來文文號','舞弊人','舞弊內容'];
       if (!chkSelect(sel,seltxt)) return;     //檢核必選select是否皆已選
       else if (!chkInTxt(input,inputtxt)) return;
       if(AskInsert_Permission())
	   {
		 form.action="/pages/MC012W.jsp?act=Insert";	    
		 form.submit();
       }
    }
    else if(cnd =='modify'){	     
	     if(AskEdit_Permission()){
	        form.action="/pages/MC012W.jsp?act=Update";	    
	        form.submit();
	     }
   }
   else if(cnd =='delete'){
	       if(AskDelete_Permission()){
	          form.action="/pages/MC012W.jsp?act=Delete";	    
	          form.submit();
	      }
   }
   else if(cnd == "Qry") {
...
{
	if(form.begY.value != "") {
        form.emDate.value = mergeDate(form.begY.value, form.begM.value, form.begD.value);
        //alert(form.VIOLATE_DATE.value);
        if(!fnValidDate(form.emDate.value)) {
            alert("來文日期不合法");
            return false;
        }
   }
