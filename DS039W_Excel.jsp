<%
//100.07.28 create 利率風險-市場風險應計提資本彙總表(6-A) by 2295
...
<%
	...
	try{	      	
			...
			titleName += "利率風險-市場風險應計提資本彙總表(6-A)";	  		
			//年
            ...
            from (
            	  ...
            	  from ( select m_year,m_month,bank_code,acc_code,'0' as type,amt,data_extra from agribank_rpt
            	    	 where (to_char(m_year * 100 + m_month) >= 9812 and to_char(m_year * 100 + m_month) <= 9912)
            		  	 ...
            		  	 and acc_tr_type = '6-A'           		  		  
            		  	 --and acc_code in ('001000','111000')
						 order by m_year,m_month,data_extra
            			)		  
				  where data_extra <> 999						
			...
			boolean sum_check = false;
			
			if(selectacc_code.indexOf("150000") != -1 || selectacc_code.indexOf("160000") != -1	
			|| selectacc_code.indexOf("170000") != -1 || selectacc_code.indexOf("180000") != -1	){
               sum_check = true;			
			}           
			...
			main_table.append(" 		 ) ");	
			if(!sum_check){//100.07.28沒有選取到合計欄位(150000,160000,170000,180000)
			main_table.append("  where data_extra <> 999 ");			
			}
            ...
            */                                   
			...
			if(dbData == null || dbData.size() == 0){
                ...
                for(i=0;i<dbData.size();i++){            
                    ...
                    for(int cellIdx =2;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                         ...
                         if("001000".equals(cell.getStringCellValue())){    
                            amt_unit = parseAgriBankRpt.getDollar(amt);//取得幣別名稱                            
                         }else{
                            amt_unit = "";
                         }
                         if("141000".equals(cell.getStringCellValue())){//匯率                               
            ...
            for(i=0;i<btnFieldList_data.size();i++){
				...
				//合計部份				
                //if(((String)((List)btnFieldList_data.get(i)).get(0)).equals("150000") ||
                //   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("152000")){
