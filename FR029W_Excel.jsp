...
//99.04.09 fix 修改部分程式改為共用 by 2808
...
<%@ page import="com.tradevan.util.report.RptFR029W" %>
...
<%
   ...
   act = dataMap.get("act")==null ? "download" : dataMap.get("act").toString();			  
   String Unit = dataMap.get("Unit")==null?"1" : dataMap.get("Unit").toString() ;			  
   ...
   String filename=(bank_type.equals("6"))?"全體農會信用部本期損益或淨值為負單位明細表.xls":"全體漁會信用部本期損益或淨值為負單位明細表.xls";
   ...
   if(!Utility.CheckPermission(request,report_no)){//無權限時,導向到LoginError.jsp
	    	...
	    	if(act.equals("Qry")){
	    	   ...
	    	   request.setAttribute("TBank", Utility.getALLTBank(bank_type));//共用元件所需參數
	       	   request.setAttribute("City", Utility.getCity());//共用元件所需參數
   			...
   			if(act.equals("view") || act.equals("download")){
   				try{		    
	    			actMsg = RptFR029W.createRpt(S_YEAR,S_MONTH,Unit,datestate,bank_type);	
...
<%!
    private final static String report_no = "FR029W" ;
