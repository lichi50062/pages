...
<%
	...
	if(DetailList!=null && DetailList.size()>0) {
	    ...
	    for(int i=0;i< DetailList.size(); i++) {
	        ...
	        out.println("<non_loan_amt>"+Utility.setCommaFormat(bean.getValue("non_loan_amt").toString())+"</non_loan_amt>");
	        out.println("<pay_amt>"+Utility.setCommaFormat(bean.getValue("pay_amt").toString())+"</pay_amt>");
	        out.println("<over_amt>"+bean.getValue("over_amt").toString()+"</over_amt>");
	        out.println("</data>");
	    }
	    out.println("</datalist>\n</xml>");
	    // XML Ducument for end
    }
	String chkSeqs = "",chkAmts="";
	String ex_Type = "FEB",ex_No = "",bank_No = "";
    String docY="",docM="",docD="",payY="",payM="",payD="";	
    String ex_Type_Name="",bank_Name="",ex_No_List="";
    String doc_Date="",docNo="",case_Status="";
    String def_Seq="",loan_Name="",loan_Date="",loan_Item="",loan_Item_Name="",loan_Amt="",def_Type="",non_Loan_Amt="",refund_Amt="",pay_Amt="",ag_Flag="";
    String lName="";String lDate="";String lItem="";String lAmt="";String lrAmt="";
    ...
    if(EditInfo != null && EditInfo.size() > 0 ){
		for(int i=0;i<EditInfo.size();i++){
			...
			non_Loan_Amt=b.getValue("non_loan_amt")==null?"":b.getValue("non_loan_amt").toString();
			...
			refund_Amt = b.getValue("refund_amt")==null?"":b.getValue("refund_amt").toString();
...
<input type='hidden' name='refund_Amt' value='<%=chkAmts%>'>
...
<tr class="sbody" bgcolor="#BFDFAE" >
	<td width='10%' align='left' rowspan='999'>需退還補貼息個案明細</td>
	<td width='8%' align='left'>借款人名稱</td>
	<td width='6%' align='left'>貸款日期</td>
	<td width='12%' align='left'>貸款種類</td>
	<td width='8%' align='left'>貸款金額(元)</td>
	<td width='22%' align='left'>缺失情節</td>
	<td width='14%' align='left'>不符規定情形</td>
	<td width='12%' align='left'>繳還補貼息金額(元)</td>
	<td align='left'>退還金額(元)</td>
</tr>
