...
<%@ page import="com.tradevan.util.report.RptTM004W" %>
...
<%
   ...
   if(session.getAttribute("muser_id")==null){//session timeout		
      System.out.println("TM004W login timeout");   
   		...
   		if(!CheckPermission(request)){//無權限時,導向到LoginError.jsp
	    	...
	    	if(act.equals("Qry")){
   			...
   			if(act.equals("view") || act.equals("download")){
   				try{		    
	    			actMsg=RptTM004W.createRpt(acc_Tr_Type,acc_Div,Unit);	    
...
<%!
    ...
    private final static String QryPgName="/pages/TM004W_Qry.jsp";        
    private final static String RptCreatePgName="/pages/TM004W_Excel.jsp";        
    private final static String LoginErrorPgName="/pages/LoginError.jsp";
    private boolean CheckPermission(HttpServletRequest request){//檢核權限    	        		
    	    boolean CheckOK=false;
    	    HttpSession session=request.getSession();            
            Properties permission=( session.getAttribute("TM004W")==null ) ? new Properties() : (Properties)session.getAttribute("TM004W");				                
            if(permission==null){
              System.out.println("TM004W.permission==null");
            }else{
               System.out.println("TM004W.permission.size="+permission.size());
