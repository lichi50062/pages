<%
// 99.09.02 create 合格自有資本與風險性資產比率計算表(1-A1) by 2295
// 99.12.28 add 026000普通股占風險性資產比率 by 2295
...
<%
	...
	try{	      	
			...
			titleName += "合格自有資本與風險性資產比率計算表(1-A1)";		  		
			//年
            ...
            from (
            	  ...
            	  from ( select m_year,m_month,bank_code,acc_code,'0' as type,amt from agribank_rpt
            	    	 where (to_char(m_year * 100 + m_month) >= 9812 and to_char(m_year * 100 + m_month) <= 9812)
            		  	 and bank_code in  ('0180012')      
            		  	 and acc_tr_type = '1-A1'        		  		  
			...
			main_table.append("  select m_year,m_month,bank_code"+field_sum);			
 			main_table.append("  from ( select m_year,m_month,bank_code,acc_code,amt from agribank_rpt ");
  			//main_table.append("          where (to_char(m_year * 100 + m_month) >= ? and to_char(m_year * 100 + m_month) <= ?)");	//101.11.05
			...
			if(dbData == null || dbData.size() == 0){
                ...
                for(i=0;i<dbData.size();i++){            
                    ...
                    for(int cellIdx =2;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                         ...
                         //024000合格自有資本與風險性資產比率
                         //025000第一類資本占風險性資產比率
                         //99.12.28 add 026000普通股占風險性資產比率                           
                         if(cell.getStringCellValue().trim().equals("024000") || cell.getStringCellValue().trim().equals("025000") || cell.getStringCellValue().trim().equals("026000")){
                            //System.out.println("get Percent");
                            amt = Utility.getPercentNumber_2(amt);
                         }else{
