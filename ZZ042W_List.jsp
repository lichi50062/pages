...
//111.02.17 fix 農(漁)會別/縣市別/列表順序-無法挑選 by 2295
%>
...
<%
	...
	System.out.println("ZZ042W_List.act="+act+":bank_type="+bank_type+":szrpt_code="+szrpt_code+":szrpt_output_type="+szrpt_output_type+":szhsien_id="+szhsien_id+":szrpt_sort="+szrpt_sort+":printStyle"+printStyle);	
	...
	System.out.println("ZZ042W_List.S_YEAR="+S_YEAR+":S_MONTH="+S_MONTH+":E_YEAR="+E_YEAR+":E_MONTH="+E_MONTH+":firstStatus="+firstStatus);		    
    ...
    for(int i=0;i< rptname_list.size(); i++) {
        bean =(DataObject)rptname_list.get(i);
        out.print("<data>");        
    ...
    for(int i=0;i< hsien_id_data.size(); i++) {
        bean =(DataObject)hsien_id_data.get(i);//107.08.17 add
        out.print("<data>");        
                      ...
                      <td><table width=680 border=1 align=center cellpadding="1" cellspacing="1" class="bordercolor" >                                                    
                          ...
                          <td width='90%' class="<%=textColor%>">	
                            ...
                            if(rptname_list != null){
                             for(int i=0;i<rptname_list.size();i++){
                             	 bean = (DataObject)rptname_list.get(i);//107.08.17 add                             	
                            %>
                            <option value="<%=(String)bean.getValue("rpt_code")%>"                                                        
                            <%if(szrpt_code.equals((String)bean.getValue("rpt_code"))){ 
                                 out.print("selected");                                
                              }   
                            %>
                            ><%=(String)bean.getValue("rpt_code")%>&nbsp;<%=(String)bean.getValue("rpt_name")%></option>                            
                            <%}
                            }//end of if
                            %>
                            </select>    
                      ...
                      if(reportList != null){ %>
            				...
            				<td class="<%=listTitle%>" width="80">檔案產生日期</td>            				            				
                   			...
                   			<% }else{ 
                    		      ...
                    		      bean = (DataObject)reportList.get(i);//107.08.17 add         		                         		      
                    		...
                    		<tr class="<%=listColor%>">                       	  
                               ...
                               <td width="220">
                               <%if( bean.getValue("s_report_name") != null ) out.print("<a href='ZZ042W.jsp?act=Download&filename="+(String)bean.getValue("rpt_fname")+"&yyymm="+yyymm+"&RPT_CODE="+szrpt_code+"&printStyle="+printStyle+"'>"+(String)bean.getValue("s_report_name")+"</a>"); else out.print("&nbsp;");%>
                               </td>
            				   <td width="60">
            				   <%if( bean.getValue("s_banktype_name") != null ) out.print((String)bean.getValue("s_banktype_name")); else out.print("&nbsp;");%>            				   
            				   </td>            				
            				   <td width="60">
