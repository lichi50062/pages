...
<%@ page import="com.tradevan.util.report.RptFR056W" %>
...
<%
  ...
  String filename = "信用部主任及分部主任參訓情形總表.xls";
  ...
  if (session.getAttribute("muser_id") == null) { //session timeout
    System.out.println("FR056W login timeout");
  ...
  if (doProcess) { //若muser_id資料時,表示登入成功====================================================================
    if (!CheckPermission(request)) { //無權限時,導向到LoginError.jsp
      ...
      if (act.equals("view") || act.equals("download")) {
        try {
          actMsg = RptFR056W.createRpt(S_YEAR,S_MONTH,bank_type);
...
<%!
  ...
  private final static String QryPgName = "/pages/FR056W_Qry.jsp";
  private final static String RptCreatePgName = "/pages/FR056W_Excel.jsp";
  ...
  private boolean CheckPermission(HttpServletRequest request){//檢核權限
    ...
    if (permission == null) {
      System.out.println("FR056W.permission == null");
    } else {
      System.out.println("FR056W.permission.size =" + permission.size());
    }
    //只要有Query的權限,就可以進入畫面
