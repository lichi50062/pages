...
<%
	...
	//Properties permission = ( session.getAttribute("muser_id")==null ) ? new Properties() : (Properties)session.getAttribute("muser_id");				                
    if(session.getAttribute("muser_id")==null ){//session timeout		
      System.out.println("BR005W login timeout");   
    ...
    if(!CheckPermission(request)){//無權限時,導向到LoginError.jsp
    	...
    	if(request.getParameter("SortList")	!= null && !((String)request.getParameter("SortList")).equals("")){
    	   System.out.println("BR005W.SortList="+(String)request.getParameter("SortList"));
        ...
        if(act.equals("RptColumn")){//報表欄位              
			List FieldList = Utility.getFileData(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"WLX02.TXT");						
			...
			System.out.println("BR005W.FieldList="+FieldListString);	
    	    ...
    	    webURL_Y = "/pages/BR005W.jsp?act=RptStyle";    	       	    	       	   	   			       
...
<%! 
	private final static String report_no = "BR005W";
    ...
    private boolean CheckPermission(HttpServletRequest request){//檢核權限    	    
            ...
            Properties permission = ( session.getAttribute("BR002W")==null ) ? new Properties() : (Properties)session.getAttribute("BR002W");				                
            if(permission == null){
              System.out.println("BR002W.permission == null");
            }else{
               System.out.println("BR002W.permission.size ="+permission.size());
    ...
    private String saveReport(HttpServletRequest request,String lguser_id){
    	...
    	try{
        		...
        		File savefile = new File(Utility.getProperties("profileDir")+System.getProperty("file.separator")+ lguser_id+"_BR005W.txt");
        		if(savefile.exists()) savefile.delete();
            	FileOutputStream fos = new FileOutputStream(Utility.getProperties("profileDir")+System.getProperty("file.separator")+ lguser_id+"_BR005W.txt");
    ...
    private String readReport(HttpServletRequest request,String lguser_id){
        ...
        HttpSession session = request.getSession();      
    	try{
    		File WorkFile = new File(Utility.getProperties("profileDir")+System.getProperty("file.separator")+ lguser_id+"_BR005W.txt");	        	
			if(WorkFile.exists()){			
				FileInputStream fis = new FileInputStream(Utility.getProperties("profileDir")+System.getProperty("file.separator")+ lguser_id+"_BR005W.txt");	
