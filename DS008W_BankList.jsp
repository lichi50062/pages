...
//108.04.26 add 所有欄位移至DS_bank_no_all.include(因多個include會造成多列空白) by 2295
//111.03.28 調整Edge無法挑選縣市別/金融機構代碼無資料可挑選 by 2295
//111.03.28 調整Edge可選擇項目,dbclick時無法將項目移至已選擇項目 by 2295
//111.03.28 調整Edge>.>>.<.<<及上移.下移無作用 by 2295
%>
...
<%
	...
	String ds_memo1 = "(1)選取[全部]：係以「各縣市彙總表」之報表格式列印&nbsp;&nbsp;";//108.04.26 add		
    String ds_memo2 = "(2)選取[各信用部]：按「金融卡發卡及ATM裝設情形明細資料」的報表內容列印";//108.04.26 add		   		   	   	
	...
	String E_MONTH = (session.getAttribute("E_MONTH")==null)?MONTH:(String)session.getAttribute("E_MONTH");                              
   	...
   	String printStyle = (session.getAttribute("printStyle")==null)?"xls":(String)session.getAttribute("printStyle");//108.04.02 add	
...
function doSubmit(cnd){
   if(cnd == 'createRpt'){      
      ...
      if(!chInput(document.BankListfrm,'BankList')) return;//95.12.05 add 起始日期不可大於結束日期
