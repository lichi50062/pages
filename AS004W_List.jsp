...
<html>
	...
	<body marginwidth="0" marginheight="0" leftmargin="0" topmargin="0" leftmargin="0">
		...
		<input type='hidden' name='qSeqNo' value=''>
		...
		<table width="1073" border="0" align="left" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF">
  			<tr> 
        		...
        		<td bgcolor="#FFFFFF">
            		<table width="1073" border="0" align="left" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF">
	     				<tr> 
	           				...
	           				<td width="30%"><font color='#000000' size=4><b><center>撤銷核准項目維護作業</center></b></font> </td>
         	...
         	<tr>
         		<td bgcolor="#FFFFFF" align="left">
	         		<table width="1073" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
			 			<tr>  
			    			<div align="right">
			      				<jsp:include page="getLoginUser.jsp?width=1073" flush="true" />
			    			</div> 
			 			</tr> 
			 		</table>
			 	</td>
            ...
            %>   
					<table width="1073" border=1  align=center height="65" bgcolor="#FFFFF" class="bordercolor">
						<tr class="sbody">
	                ...
	                <%}%>
						...
						<tr class="sbody">
							<td width="118" height="1" class="<%=nameColor%>">
								總機構單位
							</td>
			...
			%>
					<table width=1071 border=1 align=center cellpadding="1" cellspacing="1" class="bordercolor">
						<tr class="sbody" bgcolor="#BFDFAE">
							<td class='<%=listTitle%>' width="3%" height="14">序號</td>
							<td class='<%=listTitle%>' width="3%" height="14">選項</td>
							<td class='<%=listTitle%>' width="7%" height="14">總機構代號</td>
							<td class='<%=listTitle%>' width="16%" height="14">總機構名稱</td>
							<td class='<%=listTitle%>' width="39%" height="14">撤銷項目</td>
							<td class='<%=listTitle%>' width="9%" height="14">撤銷日期</td>
							<td class='<%=listTitle%>' width="17%" height="14">撤銷文號</td>
						</tr>
			...
			<%	  DataObject bean =(DataObject)qDataList.get(i); %>
						<tr>
							...
							<td class="<%=listColor%>">
								<input type='checkbox' name='delBox' value='<%=Utility.getTrimString(bean.getValue("bank_code"))+","+Utility.getTrimString(bean.getValue("seq_no"))%>'/>
							</td>
							<td class="<%=listColor%>">
								<a href="javascript:goMtnPage(form,'<%=Utility.getTrimString(bean.getValue("bank_code")) %>', '<%=Utility.getTrimString(bean.getValue("seq_no")) %>')">
								<%=Utility.getTrimString(bean.getValue("bank_code")) %></a>
							</td>
							<td class="<%=listColor%>"><%=Utility.getTrimString(bean.getValue("bank_name")) %></td>
							<td class="<%=listColor%>"><%=Utility.getTrimString(bean.getValue("revoke_item")) %></td>
							<td class="<%=listColor%>"><%=Utility.getTrimString(bean.getValue("cancel_date")) %></td>
							<td class="<%=listColor%>"><%=Utility.getTrimString(bean.getValue("doc_no")) %></td>
						<tr>
			<%   } %>
			<% } %>
			<%} %>
