...
<%@ page import="com.tradevan.util.report.RptFR075W" %>
...
<%
	...
	String actMsg = "";
	String filename = "農漁會信用部財務經營報表.xls";//108.05.28 add
	//取得session資料,取得成功時,才繼續往下執行===================================================
	if(session.getAttribute("muser_id") == null){//session timeout		
	   System.out.println("FR075W login timeout");   
		...
		if(!CheckPermission(request)){//無權限時,導向到LoginError.jsp
	     	rd = application.getRequestDispatcher( LoginErrorPgName );        
				...
				if(act.equals("view") || act.equals("download")){
					try{	
						actMsg = RptFR075W.createRpt(S_YEAR,S_MONTH,BANK_NO,Unit,bank_type,(String)session.getAttribute("muser_id"));
...
<%!
    ...
    private final static String QryPgName = "/pages/FR075W_Qry.jsp";        
    private final static String RptCreatePgName = "/pages/FR075W_Excel.jsp";        
    private final static String LoginErrorPgName = "/pages/LoginError.jsp";
    private boolean CheckPermission(HttpServletRequest request){//檢核權限    	        		
    	    boolean CheckOK=false;
    	    HttpSession session = request.getSession();            
            Properties permission = ( session.getAttribute("FR075W")==null ) ? new Properties() : (Properties)session.getAttribute("FR075W");				                
            if(permission == null){
              System.out.println("FR075W.permission == null");
            }else{
               System.out.println("FR075W.permission.size ="+permission.size());
