...
<%
   ...
   String printStyle = "xls";//輸出格式 108.03.22 add 	     
   //輸出格式 108.03.22 add
   ...
   System.out.println("printStyle1="+(String)request.getParameter("printStyle"));
   if(act.equals("view")){
...
<%
	...
	String report_no = "DS066W";
    String titleName = Utility.getPgName(report_no);
	//String titleName = "建築貸款及農地放款統計資料";
	...
	try{
		...
		//	}
    	    ...
    	    */
			...
			//prop_column.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"a13_"+bank_type+"_detail"+str9701+".TXT"));			
			prop_column.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A13_column.TXT"));			
			//=======================================================================================================================			
			           ...
			           columnLength += detail_column.size();//累加總欄位個數
              		   for(j=0;j<detail_column.size();j++){
            	 		  ...
            	 		  // if(((String)detail_column.get(j)).length() <= 6){
            	 		      ...
            	 		      a13_field += ",a13.amt"+(String)detail_column.get(j)+",a13.type_"+(String)detail_column.get(j);            	 		      
            	 		      if(a13_field_sum.indexOf((String)detail_column.get(j)) == -1){//95.10.03 fix 若不存在時,才加入sum的acc_code裡
            	 		         a13_field_sum += " ,sum(decode(acc_code,'"+(String)detail_column.get(j)+"',amt,0))";
            	 		         if("995400".equals((String)detail_column.get(j)) || "993810".equals((String)detail_column.get(j))){
            	 		        	a13_field_sum += "/1000";
            	 		         }
            	 		         a13_field_sum += " as amt"+(String)detail_column.get(j)+", ";
            	 		         if("995400".equals((String)detail_column.get(j)) || "993810".equals((String)detail_column.get(j))){
            	 		        	a13_field_sum += " '4' as type_"+(String)detail_column.get(j);
            	 		         }else{
            	 		        	a13_field_sum += " '1' as type_"+(String)detail_column.get(j);
            	 		         }
            	 		      }
			...
			prop_column_name.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A13_column.TXT"));			
			//====================================================================================================
           ...
           /* if(SortList_data != null && SortList_data.size() != 0){
			...
			//condition += " and bn01.bn_type "+(CANCEL_NO.equals("N")?"<>":"=")+" '2' and a13.bank_code = bn01.bank_no ";		  	 
			//======================================================
            ...
            //List a13_table_paramList = new ArrayList();
            //StringBuffer a13_table=new StringBuffer();//a13
            
            sqlCmd.append("select a13.bank_code,bn01.bank_name,a13.m_year || '/' || m_month as m_yearmonth,wlx01.hsien_id,cd01.hsien_name ");
			sqlCmd.append(a13_field);
			sqlCmd.append(" from ( select m_year,m_month,bank_code ");
			sqlCmd.append(a13_field_sum);
			sqlCmd.append("      from (select * ");
			sqlCmd.append("	     from ( select * from a13 ");                            
			...
			sqlCmd.append("		           and a13.acc_code in ("+selectacc_code+") ");
			sqlCmd.append(" 	       ) ");
			sqlCmd.append("     	   ) ");
			sqlCmd.append("		  group by m_year,m_month,bank_code ");
			sqlCmd.append("		)a13 ");
			sqlCmd.append("		,(select * from bn01 where bn01.m_year=100)bn01 left join (select * from wlx01 where m_year=100)wlx01 ");                
			sqlCmd.append("            on bn01.bank_no = wlx01.bank_no ");
			sqlCmd.append("            left join cd01 on wlx01.HSIEN_ID = cd01.HSIEN_ID "); 
			sqlCmd.append("            where a13.bank_code = bn01.bank_no ");
			sqlCmd.append("            and bn01.bank_no in ("+selectBank_no+") and bn01.bn_type <> '2' ");
			sqlCmd.append("            and a13.bank_code = bn01.bank_no ");
			sqlCmd.append("            group by a13.bank_code,bn01.bank_name,a13.m_year,a13.m_month,wlx01.hsien_id,cd01.hsien_name,").append(ori_field);
			if(!"".equals(order)){	  
			    //100.05.12 fix 有挑選排序欄位年月時,查詢SQL error by 2295 						    	    
			    sqlCmd.append(" order by "+order.replaceAll("field_", "amt")); 
  				//SoryBy=asc/desc
  		    	...
  		    	sqlCmd.append(" order by a13.bank_code,a13.m_year,a13.m_month ");
  		    }	
			
			
            //讀取報表欄位名稱===================================================================================        	
        	Properties p = new Properties();
			p.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A13_column.TXT"));			
			//====================================================================================================
            ...
            wb.setRepeatingRowsAndColumns(0, 1, columnLength+3, 1, 7); //設定表頭 為固定 先設欄的起始再設列的起始
  			...
  			List dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(),sqlCmd_paramList,ori_field+"m_yearmonth");
            ...
            for(i=0;i<dbData.size();i++){            
                ...
                for(int cellIdx =4;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                     ...
                     //amt = (String)a13Data.get(cell.getStringCellValue());
