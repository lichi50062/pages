<%
//100.08.08 create 選擇權採用簡易法計提資本計算表(6-E)  by 2295
...
<%@ page import="com.tradevan.util.transfer.parseAgriBankRpt" %>
...
<%
   ...
   String printStyle = "";//輸出格式 108.06.13 add
   //輸出格式 108.06.03 add
...
<%
	...
	try{	      	
			...
			System.out.println("select acc_code="+selectacc_code);        		
  			...
  			/*
  			//按bank_code,m_year,m_month,data_extra(資料年度)排序
            select bank_code,decode(length(m_year),2,'0')||m_year || '/' || decode(length(m_month),1,'0')||m_month as m_yearmonth,       
                   amt001000,amt111000
            from (
            	  select m_year,m_month,bank_code,data_extra,	       
            			 sum(decode(acc_code,'001000',amt,0)) as amt001000,
            			 sum(decode(acc_code,'111000',amt,0)) as amt111000, 
            			 '0' as type_001000,
            			 '0' as type_111000
            	  from ( select m_year,m_month,bank_code,acc_code,'0' as type,amt,data_extra from agribank_rpt
            	    	 where (to_char(m_year * 100 + m_month) >= 9812 and to_char(m_year * 100 + m_month) <= 9912)
            		  	 and bank_code in  ('0180012')     
            		  	 and acc_tr_type = '6-E'
						 order by m_year,m_month,data_extra
            			)	  
				  					
            	  group by m_year,m_month,bank_code,data_extra
            	 )agribank_rpt
            group by bank_code,m_year,m_month,amt001000,amt111000,type_001000,type_111000
			...
			main_table.append("  select m_year,m_month,bank_code"+field_sum);			
 			main_table.append("  from ( select m_year,m_month,bank_code,acc_code,amt,data_extra from agribank_rpt ");
			...
			main_table.append(" 		 ) ");				
			main_table.append("  group by m_year,m_month,bank_code,data_extra ");
            ...
            */                                   
			...
			if(dbData == null || dbData.size() == 0){
                ...
                for(i=0;i<dbData.size();i++){            
                    ...
                    for(int cellIdx =2;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                         ...
                         if("001000".equals(cell.getStringCellValue()) || "002000".equals(cell.getStringCellValue())){    
                            amt_unit = parseAgriBankRpt.getDollar(amt);//取得幣別                            
                         }else{
                            amt_unit = "";
                         }
                         ...
                         if("".equals(amt_unit)){                         
                            ...
                            reportUtil.createCell( wb, row, ( short )columnIdx, amt_unit, rightStyle );//幣別               
                         }
                         columnIdx ++;
                    }
                    columnIdx = 1;
			...
			//99.10.29 合併acc_code時,科目代號跟欄位名稱時,合併後的列會縮小			
            for(i=0;i<btnFieldList_data.size();i++){
                ...
                if(((String)((List)btnFieldList_data.get(i)).get(0)).equals("001000") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("151100") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("151200") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("151300") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("151400") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("151500") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("002000") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("251100") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("251200") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("251300") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("251400") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("251500") ||
                   ((String)((List)btnFieldList_data.get(i)).get(0)).equals("100000")){
