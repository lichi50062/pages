...
<%@ page import="com.tradevan.util.report.RptFR057W" %>
...
<%
  ...
  String filename = "全體農漁會信用部從業人員參加金融相關業務進修情形統計表.xls";
  ...
  if (session.getAttribute("muser_id") == null) { //session timeout
    System.out.println("FR057W login timeout");
  ...
  if (doProcess) { //若muser_id資料時,表示登入成功====================================================================
    if (!CheckPermission(request)) { //無權限時,導向到LoginError.jsp
      ...
      if (act.equals("view") || act.equals("download")) {
        try {
          actMsg = RptFR057W.createRpt(S_YEAR,S_MONTH,bank_type);
...
<%!
  ...
  private final static String QryPgName = "/pages/FR057W_Qry.jsp";
  private final static String RptCreatePgName = "/pages/FR057W_Excel.jsp";
  private final static String LoginErrorPgName = "/pages/LoginError.jsp";
  private boolean CheckPermission(HttpServletRequest request) {//檢核權限
    boolean CheckOK = false;
    HttpSession session = request.getSession();
    Properties permission = ( session.getAttribute("FR057W")==null ) ? new Properties() : (Properties)session.getAttribute("FR057W");

    if (permission == null) {
      System.out.println("FR057W.permission == null");
    }   else {
      System.out.println("FR057W.permission.size ="+permission.size());
    }
    //只要有Query的權限,就可以進入畫面
