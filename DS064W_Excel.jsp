...
<%
	...
	try{
			...
			sqlCmd.append("        		decode(field_loan_bal_cnt,0,0,round(field_again_loan_cnt / field_loan_bal_cnt *100 ,2)) as field_loan_bal_cnt_rate,");
			sqlCmd.append("        		decode(field_loan_bal_amt,0,0,round(field_again_loan_bal_amt / field_loan_bal_amt *100 ,2)) as field_loan_bal_amt_rate,");
			sqlCmd.append("        		decode(field_loan_bal_amt_sum,0,0,round(field_again_loan_bal_amt / field_loan_bal_amt_sum *100 ,4)) as field_loan_bal_amt_sum_rate,");
			...
			sqlCmd.append("        		'4' as type_loan_bal_cnt_rate,");
			sqlCmd.append("        		'4' as type_loan_bal_amt_rate,");
			sqlCmd.append("        		'4' as type_loan_bal_amt_sum_rate ");
			sqlCmd.append(" 	   from (select agri_loan.m_year,agri_loan.m_month, agri_loan.bank_code ,");
			sqlCmd.append("               		round(sum(agri_loan.loan_bal_cnt) /1,0) as field_loan_bal_cnt,");
			sqlCmd.append("               		round(sum(agri_loan.loan_bal_amt) /1,0) as field_loan_bal_amt,");
			sqlCmd.append("               		round(sum(agri_loan.again_loan_cnt) /1,0) as field_again_loan_cnt,");
			sqlCmd.append("               		round(sum(agri_loan.again_loan_bal_amt) /1,0) as field_again_loan_bal_amt,");
			sqlCmd.append("               		round(sum(agri_loan.again_loan_bal_amt_1th) /1,0) as field_again_loan_bal_amt_1th ");
			...
			sqlCmd.append("         	(select round(sum(agri_loan.loan_bal_amt) /1,0) as field_loan_bal_amt_sum ");          
			sqlCmd.append("         	   from agri_loan "); 
			sqlCmd.append("         	  where m_year=? and m_month=? ");
			sqlCmd_paramList.add(S_YEAR);
			sqlCmd_paramList.add(S_MONTH);
            ...
            for(i=0;i<dbData.size();i++){            
                ...
                for(int cellIdx =4;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                     ...
                     if(!(amt_type.equals("4") || "again_loan_cnt".equals(tmpStr) || "loan_bal_cnt".equals(tmpStr))){//該值不為利率或戶數時.再除以單位4捨五入
