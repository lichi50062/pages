...
<%
	String title = "某年底全體農漁會信用部存款、放款、資產總額、淨值、本期損益及淨值與存款總額比率排名表";
	Calendar rightNow = Calendar.getInstance();
   	Calendar now = Calendar.getInstance();
   	String YEAR  = String.valueOf((now.get(Calendar.YEAR)-1911)-1); //回覆值為西元年故需-1911取得民國年;
   	
 	//取得權限
	Properties permission = ( session.getAttribute("AN011W")==null ) ? new Properties() : (Properties)session.getAttribute("AN011W"); 
	if(permission == null){
       System.out.println("AN011W_Qry.permission == null");
    }else{
       System.out.println("AN011W_Qry.permission.size ="+permission.size());               
    }
%>
...
function doSubmit(){  
	...
	if(document.form.bankType.value == "6"){
		if (!confirm("本項報表會執行 60-80 秒，是否確定執行？")){return;}
	}else{
		if (!confirm("本項報表會執行 35-50 秒，是否確定執行？")){return;}
	}
	this.document.forms[0].action = "/pages/report/AN011W.jsp?act=Excel";
