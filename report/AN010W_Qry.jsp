...
<%
	String title = "某年底全體農漁會信用部各類財務比率";
	Calendar rightNow = Calendar.getInstance();
   	Calendar now = Calendar.getInstance();
   	String YEAR  = String.valueOf((now.get(Calendar.YEAR)-1911)-1); //回覆值為西元年故需-1911取得民國年;
 	//取得權限
	Properties permission = ( session.getAttribute("AN010W")==null ) ? new Properties() : (Properties)session.getAttribute("AN010W"); 
	if(permission == null){
       System.out.println("AN010W_Qry.permission == null");
    }else{
       System.out.println("AN010W_Qry.permission.size ="+permission.size());               
    }
%>
...
<!--
function doSubmit(){ 
	if(document.form.S_YEAR.value > <%=YEAR%>){
		alert("本表僅供查詢"+<%=YEAR%>+"年度(含)以前資料");
		document.form.S_YEAR.focus();
		return;
	}
	if (!confirm("本項報表會執行 15-20 秒，是否確定執行？")){return;} 
	this.document.forms[0].action = "/pages/report/AN010W.jsp?act=Excel";
