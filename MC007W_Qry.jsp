...
<%!	private final String report_no = "MC007W" ;
//取得所有總機構資料
...
<%	
    ...
    String tbank = "";
    List tBankList = this.getTatolBankType();
...
function doSubmit(){   
  if((this.document.forms[0].rptStyle.value == '0' || this.document.forms[0].rptStyle.value == '1') && this.document.forms[0].RptYear.value == ""){//全體明細表.各縣市總表
     alert("年報年份不可為空白!!");
     this.document.forms[0].RptYear.focus(); 
     return;
  }
  if(this.document.forms[0].rptStyle.value == '1' && this.document.forms[0].cityType.value == ""){//各縣市總表
     alert("縣市別不可為空白!!");
     this.document.forms[0].cityType.focus(); 
     return;
  }
  if(this.document.forms[0].rptStyle.value == '2' && this.document.forms[0].tbank.value == ""){//單一家信用部歷年明細表
     alert("機構名稱必需選擇單一家信用部!!");
     this.document.forms[0].tbank.focus(); 
     return;
  }
  if(confirm("本項報表會報行10-15秒，是否確定執行？"))
  {
        this.document.forms[0].action = "/pages/MC007W_Excel.jsp";	
...
function checkflag(){
   if(this.document.forms[0].rptStyle.value == '0'){//全體農漁會明細表
	  if(this.document.forms[0].RptYear.value==''){
	  	this.document.forms[0].RptYear.value = '<%=Utility.getYear() %>';
	  }
      this.document.forms[0].RptYear.disabled = false; 
      this.document.forms[0].bankType.disabled = true; 
      this.document.forms[0].cityType.disabled = true; 
      this.document.forms[0].tbank.disabled = true;  
   }else if(this.document.forms[0].rptStyle.value == '1'){//各縣市總表
	  if(this.document.forms[0].RptYear.value==''){
	  	this.document.forms[0].RptYear.value = '<%=Utility.getYear() %>';
	  }
      this.document.forms[0].RptYear.disabled = false; 
      this.document.forms[0].bankType.disabled = false; 
      this.document.forms[0].cityType.disabled = false; 
      this.document.forms[0].tbank.disabled = true;        
   }else if(this.document.forms[0].rptStyle.value == '2'){//單一家信用部歷年明細表
      this.document.forms[0].RptYear.value = "";
      this.document.forms[0].RptYear.disabled = true; 
      this.document.forms[0].bankType.disabled = false; 
      this.document.forms[0].cityType.disabled = false; 
      this.document.forms[0].tbank.disabled = false;          
   }
   chnageYear();
}
//組縣市別============
function changeCity(xml) {
	...
	//1.取得畫面年分 
	var begY = form.RptYear.value=='' ? 0 : eval(form.RptYear.value) ;
	Myear = '100' ;//預設年分100年
	if(begY !=0 && begY<=99) {
		Myear = '99' ;
	}
...
<Form name='form' method=post action='MC007W.jsp'>
          ...
          <td><table width="600" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr> 
                ...
                <td width="*"><font color='#000000' size=4> 
                  <center>
                    <font color="#336600">農漁會信用部簽証會計師明細表</font>
                  </center>
                  </font> </td>
                <td width="170"><img src="images/banner_bg1.gif" width="170" height="17"></td>
              </tr>
          ...
          <td><table width="600" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#5DA525">
              <tr> 
                <td bordercolor="#E9F4E3" bgcolor="#E9F4E3"><table width="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#E9F4E3">
                     ...
                     <tr class='sbody'>
						<td><img src="images/2_icon_01.gif" width="16" height="16" align="absmiddle"><span class="mtext">報表格式 :</span>
                          <select size="1" name="rptStyle" onChange="checkflag()">                         
                             <option value ='0' selected>全體農漁會信用部明細表</option>
                             <option value ='1' >各縣市總表</option>  
                             <option value ='2' >單一家信用部歷年明細表</option>                            
                    ...
                    <tr class='sbody'>
						<td>
							<img src="images/2_icon_01.gif" width="16" height="16" align="absmiddle"><span class="mtext">年度 :</span>                                
                       		 <input type='text' name='RptYear' value="<%=Utility.getYear() %>" size='3' maxlength='3' onblur='CheckYear(this)' onchange="chnageYear();"> 
                       </td>
                    </tr>  
