<%
//94/11/23 by 4180
//099.04.08 fix 修改以共用方式取代原本程式 by 2808
%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ page import="com.tradevan.util.DBManager" %>
<%@ page import="com.tradevan.util.dao.DataObject" %>
<%@ page import="com.tradevan.util.ListArray" %>
<%@ page import="com.tradevan.util.Utility" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.LinkedList" %>
<%@ page import="java.util.Properties" %>
<%@ page import="java.util.Enumeration" %>
<%@ page import="java.util.HashMap" %>
<%@ page import="java.util.Hashtable" %>
<%@ page import="java.io.FileOutputStream" %>
<%@ page import="java.io.BufferedOutputStream" %>
<%@ page import="java.io.File" %>
<%@ page import="java.io.PrintStream" %>
<%@ page import="java.io.*" %>
<%@ page import="java.util.*" %>
<%@include file="./include/Header.include" %>
<%
	//取得session資料,取得成功時,才繼續往下執行===================================================
    //若muser_id資料時,表示登入成功====================================================================	
	//99.04.08 fix Header.include取代  by 2808
	String bank_type = Utility.getTrimString(dataMap.get("bank_type"));			    	
	String firstStatus = Utility.getTrimString(dataMap.get("firstStatus"));			    	
	System.out.println("act="+act);
	System.out.println("bank_type="+bank_type);
	
	if(firstStatus.equals("true")){//若從Menu點選時,先清空session裡的資料
	   session.setAttribute("HSIEN_ID",null);   
	   session.setAttribute("BankList",null);    	
	   session.setAttribute("btnFieldList",null);	
	   session.setAttribute("nowbank_type",null);   
	   session.setAttribute("position_code",null);   
	}
	if(!Utility.CheckPermission(request,report_no)){//無權限時,導向到LoginError.jsp
        rd = application.getRequestDispatcher( LoginErrorPgName );        
    }else{            
    	//set next jsp 	
    	//將選取的縣市別,金融機構代碼,報表欄位,排序的報表欄位寫到session=======================================    
    	if( (String)dataMap.get("BankList")	!= null && !"".equals((String)dataMap.get("BankList"))){
    	   session.setAttribute("BankList",(String)dataMap.get("BankList"));    
    	   String BankList_data = "BankList="+(String)session.getAttribute("BankList");	
    	}   
    	if((String)dataMap.get("btnFieldList")!= null && !"".equals((String)dataMap.get("btnFieldList"))){
    	   session.setAttribute("btnFieldList",(String)dataMap.get("btnFieldList"));	
    	}
    	
        if((String)dataMap.get("HSIEN_ID")!= null && !"".equals((String)dataMap.get("HSIEN_ID") )){
           session.setAttribute("HSIEN_ID",(String)dataMap.get("HSIEN_ID"));   
        }
       
        if((String)dataMap.get("bank_type")!= null && !"".equals((String)dataMap.get("bank_type"))){
           session.setAttribute("nowbank_type",(String)dataMap.get("bank_type"));   
        }
        //=================================================================================================
               
        if(act.equals("Qry")){//金融機構
            System.out.println("btnFieldList="+(String)dataMap.get("btnFieldList"));            
            rd = application.getRequestDispatcher( BankListPgName );                 
        }else if(act.equals("createRpt")){//產生Excel報表
            String excelAction = (String)dataMap.get("excelaction");
            rd = application.getRequestDispatcher( RptCreatePgName +"?act="+excelAction);         
        }
        
    	request.setAttribute("actMsg",actMsg);    
    	request.setAttribute("alertMsg",alertMsg);
    	request.setAttribute("webURL_Y",webURL_Y);
    	request.setAttribute("webURL_N",webURL_N);   
    	request.setAttribute("TBank", Utility.getALLTBank(bank_type));
    	request.setAttribute("City", Utility.getCity());
        
    }        
     
%>
<%@include file="./include/Tail.include" %>

<%!
    private final static String report_no = "FR025WA" ;
    private final static String nextPgName = "/pages/ActMsg.jsp";    
    private final static String BankListPgName = "/pages/"+report_no+"_Qry.jsp";          
    private final static String RptCreatePgName = "/pages/"+report_no+"_Excel.jsp";        
    private final static String LoginErrorPgName = "/pages/LoginError.jsp";
    
   %>