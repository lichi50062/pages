<%
//95.07.24 信用部存款餘額表/放款餘額表 create by 2295
...
<%@ page import="com.tradevan.util.report.RptFR039W" %>
<%@ page import="com.tradevan.util.report.RptFR039WB" %>
...
<%
   ...
   if(session.getAttribute("muser_id") == null){//session timeout
      System.out.println("FR039W login timeout");
   			...
   			if(act.equals("view") || act.equals("download")){
   				try{
   				    if("0".equals(rptStyle)){
   				     	actMsg = RptFR039W.createRpt(S_YEAR,S_MONTH,Unit,bank_type);
   				    }else{
   				     	actMsg = RptFR039WB.createRpt(S_YEAR,S_MONTH,Unit,bank_type);
   				    }
	    			//actMsg = RptFR039W.createRpt(S_YEAR,S_MONTH,bank_type,Unit);	    			
	    			//actMsg = RptFR039WB.createRpt(S_YEAR,S_MONTH,Unit,bank_type,rptStyle);
...
<%!
    ...
    private final static String QryPgName = "/pages/FR039W_Qry.jsp";
    private final static String RptCreatePgName = "/pages/FR039W_Excel.jsp";
    private final static String LoginErrorPgName = "/pages/LoginError.jsp";
    private boolean CheckPermission(HttpServletRequest request){//檢核權限
    	    boolean CheckOK=false;
    	    HttpSession session = request.getSession();
            Properties permission = ( session.getAttribute("FR039W")==null ) ? new Properties() : (Properties)session.getAttribute("FR039W");
            if(permission == null){
              System.out.println("FR039W.permission == null");
            }else{
               System.out.println("FR039W.permission.size ="+permission.size());
