...
<%@ page import="com.tradevan.util.report.RptTM009W" %>
...
<%
   ...
   if(session.getAttribute("muser_id")==null){//session timeout		
      System.out.println("TM009W login timeout");   
   ...
   if(doProcess){//若muser_id資料時,表示登入成功====================================================================	
   		if(!CheckPermission(request)){//無權限時,導向到LoginError.jsp
	    	...
	    	if(act.equals("Qry")){
				...
				request.setAttribute("AllDates", getAllDates());
   			...
   			if(act.equals("view") || act.equals("download")){
   				try{		    
	    			actMsg=RptTM009W.createRpt(acc_Tr_Type, acc_Div, Unit, applyDate);	    
...
<%!
    ...
    private final static String QryPgName="/pages/TM009W_Qry.jsp";        
    private final static String RptCreatePgName="/pages/TM009W_Excel.jsp";        
    private final static String LoginErrorPgName="/pages/LoginError.jsp";
    private boolean CheckPermission(HttpServletRequest request){//檢核權限    	        		
    	    boolean CheckOK=false;
    	    HttpSession session=request.getSession();  
    	    
            Properties permission=( session.getAttribute("TM009W")==null ) ? new Properties() : (Properties)session.getAttribute("TM009W");				                
            if(permission==null){
              System.out.println("TM009W.permission==null");
            }else{
               System.out.println("TM009W.permission.size="+permission.size());
