...
<body>
產生(農.漁會-各別機構)WR00_operation
<%
   System.out.println("=============產生WR00_Operation開始===========");
   ...
   System.out.println("=============產生WR00_Operation結束===========");      
%>
...
public String Generate(String Report_no,String s_year,String s_month,String isDebug,String lguser_id) throws Exception{    	
    	...
    	String last_year = "";
    	String last_month = "";
    	String wr_rpt = "0";//警示報表類別 0:與上月比較 1:與上季比較 2:與上年度同期比較
...
try{      
    ...
    if(dbData.size() > 0){
		...
		updateDBSqlList.add(updateDBDataList);//0:sql 1:參數List
		updateDBList.add(updateDBSqlList);
	  	//增加建築放款加總/上年度信用部決算淨值平均數.逾放比率平均數
	  	List getAVG = getAVG(s_year,s_month,last_year,last_month,wlx01_m_year);
	  	if(isDebug.equals("true"))  System.out.println("***********getAVG.size()="+getAVG.size());
	  	String field_992710_990230_rate_avg = "";
	  	String field_over_rate_avg = "";
	  	if(getAVG.size()>0){
	  		AVGBean = (DataObject) getAVG.get(0);
	  		field_992710_990230_rate_avg = (AVGBean.getValue("field_992710_990230_rate_avg")).toString();
    	...
    	updateDBDataList = new ArrayList();
	  	sqlCmd.delete(0, sqlCmd.length());
		dataList = new ArrayList();//傳內的參數List
		sqlCmd.append("insert into WR_OPERATION(M_YEAR,M_MONTH,WR_RPT,BANK_CODE,ACC_CODE,TYPE,AMT,WARN_TYPE,REMARK,WR_RANGE_SERIAL) values(?,?,?,?,?,?,?,?,?,?)");
		dataList.add(s_year); 
	    dataList.add(s_month); 
	    dataList.add(wr_rpt); 
	    dataList.add("ALL");
	    dataList.add("field_992710_990230_rate_avg"); 
	    dataList.add("4"); 
	    dataList.add(field_992710_990230_rate_avg);
		dataList.add("");
	    dataList.add(""); 
		...
		if(!DBManager.updateDB_ps(updateDBList)){					 
			...
			logps.println(Utility.getDateFormat("yyyy/MM/dd  HH:mm:ss  ")+" "+"執行 WR00_opeation與上月比較 失敗");
		}
		else{
			logps.println(Utility.getDateFormat("yyyy/MM/dd  HH:mm:ss  ")+" "+"產生 WR00_opeation與上月比較 完成");				   
...
private List getAVG(String m_year,String m_month,String last_year,String last_month,String wlx01_m_year){
    StringBuffer sqlCmd = new StringBuffer();        	
	List paramList = new ArrayList();				
	sqlCmd.append(" select  decode(a01.field_990230,0,0,round(a01.field_992710 /  a01.field_990230 *100 ,2))  as  field_992710_990230_RATE_AVG,");//--平均數.所有農漁會建築放款加總/所有農漁會上年度信用部決算淨值
	sqlCmd.append("         decode(a01.field_CREDIT,0,0,round(a01.field_OVER /  a01.field_CREDIT *100 ,2))  as   field_OVER_RATE_AVG ");//--逾放比率.平均數
	...
	sqlCmd.append("          from (select * from bn01 where m_year = ? and bank_type in ('6','7') and bn_type <> '2')bn01 ");
	paramList.add(wlx01_m_year);
	sqlCmd.append("          left join (select (CASE WHEN (a01.m_year <= 102) THEN '102' WHEN (a01.m_year > 102) THEN '103' ELSE '00' END) as YEAR_TYPE,m_year,m_month,bank_code,acc_code,amt from a01 where m_year=? and m_month=? ");
	paramList.add(m_year);
	paramList.add(m_month);
	sqlCmd.append("                    ) a01  on  bn01.bank_no = a01.bank_code  ");
	sqlCmd.append("          group by a01.m_year,a01.m_month,bn01.bank_no,bn01.BANK_NAME ");
	...
	sqlCmd.append("         from (select * from bn01 where m_year = ? and bank_type in ('6','7') and bn_type <> '2')bn01 ");
	paramList.add(wlx01_m_year);
	sqlCmd.append("         left join (select * from a02 where m_year = ? and m_month=? and a02.ACC_code in ('990230') ) a02 on  bn01.bank_no = a02.bank_code "); 
	paramList.add(last_year);
	paramList.add(last_month);
	sqlCmd.append("         group by a02.m_year,a02.m_month,bn01.bank_no,bn01.BANK_NAME ");
	sqlCmd.append("        ) a02,");     
	...
	sqlCmd.append("        and a01.bank_no <> ' ' ");       
	sqlCmd.append(" ) a01 ");
	List dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(),paramList,"field_992710_990230_rate_avg,field_over_rate_avg");
	return dbData;
}
