...
<%
    ...
    System.out.println("tbankbanktbankbank"+tbank);
...
<td bgcolor="#EBF4E1" height="1">
  <input type='radio' name='ex_Type' value='FEB'  onClick="changeEx_No(form);">金管會檢查報告&nbsp;
  <input type='radio' name='ex_Type' value='AGRI' onClick="changeEx_No(form);">農業金庫查核&nbsp;
  <input type='radio' name='ex_Type' value='BOAF' onClick="changeEx_No(form);">農金局訪查&nbsp;
	&nbsp;&nbsp;&nbsp;&nbsp;
...
<td bgcolor="#EBF4E1" height="1">           
  <select size="1" name="tbank" id="tbank">
...
<tr class="sbody" id="showFEB" style="display:none">
       ...
       <td bgcolor="#EBF4E1" height="1">
       <input type="text" name="ex_No" valuet='<%=ex_No %>' size="49" maxlength='10'>
    </tr> 
...
<% if(!"".equals(ex_Type)) {%>
    ...
    <tr class="sbody" bgcolor="#BFDFAE">
      <td width="30%" height="14"><%if("FEB".equals(ex_Type)){%>檢查報告編號<%}else if("AGRI".equals(ex_Type)){%>查核季別<%}else{%>訪查日期<%} %></td>
      <td width="30%" height="14">機構代號</td>          
    ...
    <tr class="sbody" bgcolor='<%=(i % 2 == 0)?"#EBF4E1":"#FFFFCC"%>'>
      <td width="30%" height="1"><a href=FL004W.jsp?act=DetailList&bank_No=<%=bean.getValue("bank_no")%>&ex_Type=<%=bean.getValue("ex_type")%>&ex_No=<%=bean.getValue("ex_no")%>>
							      <%=bean.getValue("ex_no_list") != null ? bean.getValue("ex_no_list") : "&nbsp;"%>
							      </a>&nbsp;</td>
      <td width="30%" height="1"><%=bean.getValue("bank_no") != null ? bean.getValue("bank_no") : "&nbsp;"%></td>
      <td width="40%" height="1"><%=bean.getValue("bank_name") != null ? bean.getValue("bank_name") : "&nbsp;"%></td>
    </tr>
    <%        
      }
    }else{%>            
...
<!--
if('<%=ex_Type%>' !=''){
	if('<%=ex_Type%>'=='FEB') document.form.ex_Type[0].checked=true;
	if('<%=ex_Type%>'=='AGRI') document.form.ex_Type[1].checked=true;
	if('<%=ex_Type%>'=='BOAF') document.form.ex_Type[2].checked=true;
	changeEx_No(form);
	setSelect(document.form.begM,"<%=begM%>");
	setSelect(document.form.endM,"<%=endM%>");
	setSelect(document.form.begD,"<%=begD%>");
	setSelect(document.form.endD,"<%=endD%>");
	setSelect(document.form.cityType,"<%=cityType%>");
	setSelect(document.form.begSeasonS,"<%=begSeasonS%>");
	setSelect(document.form.endSeasonS,"<%=endSeasonS%>");
}
if("<%=tbank%>"!='')setSelect(document.form.tbank,"<%=bankType%>");
changeCity();
changeTbank();
//changeOption('TBankXML',document.form.tbank, document.form.bankType, 'TBankXML');
setSelect(document.form.tbank,"<%=tbank%>");
