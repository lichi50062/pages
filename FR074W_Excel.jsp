...
<%@ page import="com.tradevan.util.report.RptFR074W" %>
...
<%
   ...
   if(session.getAttribute("muser_id")==null){//session timeout		
      System.out.println("FR074W login timeout");   
   		...
   		if(!CheckPermission(request)){//無權限時,導向到LoginError.jsp
	    	...
	    	if(act.equals("Qry")){
   			...
   			if(act.equals("view") || act.equals("download")){
   				try{		    
	    			actMsg=RptFR074W.createRpt(S_YEAR,S_MONTH,Unit);	    
...
<%!
    ...
    private final static String QryPgName="/pages/FR074W_Qry.jsp";        
    private final static String RptCreatePgName="/pages/FR074W_Excel.jsp";        
    private final static String LoginErrorPgName="/pages/LoginError.jsp";
    private boolean CheckPermission(HttpServletRequest request){//檢核權限    	        		
    	    boolean CheckOK=false;
    	    HttpSession session=request.getSession();            
            Properties permission=( session.getAttribute("FR074W")==null ) ? new Properties() : (Properties)session.getAttribute("FR074W");				                
            if(permission==null){
              System.out.println("FR074W.permission==null");
            }else{
               System.out.println("FR074W.permission.size="+permission.size());
