...
//110.02.26 add 110年4月套用新報表項目 by 2295
...
<%
	...
	String S_YEAR=Utility.getYear();//110.02.24 add
	String S_MONTH=Utility.getMonth();//110.02.24 add
	if(firstStatus.equals("true")){//若從Menu點選時,先清空session裡的資料
	   ...
	   session.setAttribute("printStyle",null); //108.03.25 add   
	}
	if(!Utility.CheckPermission(request,report_no)){//無權限時,導向到LoginError.jsp//95.11.03 fix 使用Utility.CheckPermission檢核權限 by 2295
        ...
        if(act.equals("RptColumn")){//報表欄位              
			List FieldList = Utility.getFileData(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A02_"+bank_type+"_master"+((Integer.parseInt(S_YEAR+S_MONTH) >= 11005)?"_11004":"")+".TXT");//110.02.24 fix						
			//95.09.06 add 若金融機構代號選為全部時,則讀取bank_type=ALL的設定檔//A02無選全部資料 110.02.24 
			//110.02.24 農漁會報表欄位定義檔,只有master有分.其餘內容都一樣 
			System.out.println(report_no+".bank_type="+bank_type);
			...
			System.out.println(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A02_"+bank_type+"_master"+((Integer.parseInt(S_YEAR+S_MONTH) >= 11005)?"_11004":"")+".TXT");//110.02.24 fix
