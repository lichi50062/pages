...
<%
	...
	String report_no = "FR068W";
    Map dataMap =Utility.saveSearchParameter(request);
	String S_YEAR = Utility.getTrimString(dataMap.get("S_YEAR")) ;		
	String S_MONTH = Utility.getTrimString(dataMap.get("S_MONTH")) ;			
	String bankType = Utility.getTrimString(dataMap.get("bank_type")); 
	String title=((bankType.equals("6"))?"農會":"漁會")+"信用部基層機構逾期放款及轉銷呆帳及存款準備率降低所增盈餘月報表";
    String cancel_no  = "" ;
...
function doSubmit(cnd){   
	   this.document.forms[0].action = "/pages/FR068W_Excel.jsp?act=download";	
	   this.document.forms[0].target = '_self';
	   this.document.forms[0].submit();   
}
//94.03.10 add by egg
//重導FR068W_Qry.jsp頁面以致輸出對應之金融機構資料
function getData(form,item){
	if(item == 'cancel_no'){
		//初始BANK_NO值
	   	form.BANK_NO.value="";
	}
	form.action="/pages/FR068W_Qry.jsp?act=getData&test=nothing";
	form.submit();
}
...
<Table border=1 width='600' align=center height="50" bgcolor="#FFFFF" bordercolor="#76C657">
