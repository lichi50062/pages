...
<%
	...
	if(firstStatus.equals("true")){//若從Menu點選時,先清空session裡的資料
	   session.setAttribute("S_YEAR",null);//99.12.31 add
	   session.setAttribute("S_MONTH",null);//99.12.31 add
    ...
    if(!Utility.CheckPermission(request,report_no)){//無權限時,導向到LoginError.jsp//95.11.03 fix 使用Utility.CheckPermission檢核權限 by 2295
    	...
    	if(!Utility.getTrimString(dataMap.get("SortList")).equals("")){
    	   System.out.println("BR021W.SortList="+(String)request.getParameter("SortList"));
        ...
        if(act.equals("RptColumn")){//報表欄位              
			List FieldList = Utility.getFileData(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"BANK_CMML.TXT");						
			...
			System.out.println("BR021W.FieldList="+FieldListString);	
    	    ...
    	    webURL_Y = "/pages/BR021W.jsp?act=RptStyle";    	       	    	       	   	   			       
...
<%!
     private final static String report_no = "BR021W";
