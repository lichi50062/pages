...
<%
	...
	System.out.println("Page: MC011W_List.jsp    Action:"+act);
    ...
    String[] dffeType= request.getParameterValues("dffeTypeQry")==null ? null:request.getParameterValues("dffeTypeQry");
    String[] dffeBType= request.getParameterValues("dffeBTypeQry")==null ? null:request.getParameterValues("dffeBTypeQry");
    
    String dffeTypeStr="";
    String dffeBTypeStr="";


    String dffeTitle=Utility.getTrimString(dataMap.get("dffeTitle")) ; //主旨
    String dffeContent=Utility.getTrimString(dataMap.get("dffeContent")) ; //內容
    ...
    //取得checkbox 的 value並存成字串    
    if(dffeType!=null)
    {
    	StringBuffer sb=new StringBuffer();
    	for(int i=0;i<dffeType.length-1;i++)
    	{
    		sb.append(dffeType[i]+",");
    	}
    	sb.append(dffeType[dffeType.length-1]);
    	dffeTypeStr=sb.toString();
    }
    
    if(dffeBType!=null)
    {
    	StringBuffer sb=new StringBuffer();
    	for(int i=0;i<dffeBType.length-1;i++)
    	{
    		sb.append(dffeBType[i]+",");
    	}
    	sb.append(dffeBType[dffeBType.length-1]);
    	dffeBTypeStr=sb.toString();
    }
	// 轉換西元年到民國年
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
     <tr> 
           ...
           <td width="30%"><font color='#000000' size=4><b><center>個別農漁會解釋函令查詢</center></b></font> </td>
...
<td width="416" bgcolor="#EBF4E1" height="1">
 ...
 <select size="1" name="cityType" onChange="changeTbank('TBankXML')" >
  ...
  <a href="/pages/MC011W.jsp?act=New" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image102','','images/bt_addb.gif',1)">
...
<tr class="sbody">
  ...
  <td width="540" bgcolor="#EBF4E1" height="20">
<%   List dTypeList = (List) request.getAttribute("DType");
     if(dTypeList != null) {
         for(int i=0; i < dTypeList.size(); i++) {
             DataObject bean =(DataObject)dTypeList.get(i);
%>   
     <input type="checkbox"  name="dffeTypeQry" value="<%=bean.getValue("select_num")%>"><%=bean.getValue("select_name")%>
<%       }
     }
%>  
  </td>
...
%>   
     <input type="checkbox" name="dffeBTypeQry" value="<%=bean.getValue("select_num")%>"><%=bean.getValue("select_name")%>
<%       }
     }
%>  
  </td>
...
<tr class="sbody">
  ...
  <td width="416" bgcolor="#EBF4E1" height="20">
  <input type="text" name="dffeTitle" size="20"></td>
</tr>
    
<tr class="sbody">
  <td width="118" bgcolor="#BDDE9C" height="20">內容</td>
  <td width="416" bgcolor="#EBF4E1" height="20">
  <input type="text" name="dffeContent" size="20"></td>
</tr>
</Table>
...
<Table border="1" width="100%"  bgcolor="#FFFFF" bordercolor="#76C657">
      <tr class="sbody" bgcolor="#BFDFAE">
        ...
        <td width="15%" height="14">發文日期</td>          
        <td width="20%" height="14">主旨</td>
        <td width="50%" height="14">內容</td> 
      </tr>
      ...
      <tr class="sbody" bgcolor='<%=(i % 2 == 0)?"#EBF4E1":"#FFFFCC"%>'>
        <td width="15%" height="1"><a href=MC011W.jsp?act=Edit&bank_no=<%=bean.getValue("bank_no")%>&dffeNum=<%=bean.getValue("dffe_number")%>&dffeNo=<%=bean.getValue("dffe_no")%>><%=bean.getValue("dffe_number")%></a>&nbsp;</td>
        <td width="15" height="1"><%=bean.getValue("dffe_date") != null ? bean.getValue("dffe_date") : "&nbsp;"%></td>
        <td width="20%" height="1"><%=bean.getValue("dffe_title") != null ? bean.getValue("dffe_title") : "&nbsp;"%></td>
        <td width="50%" height="1"><%=bean.getValue("dffe_content") != null ? bean.getValue("dffe_content") : "&nbsp;"%></td>
      </tr>
      <%        }
     }else {%>            
    ...
    <td width="97%"> 
      <ul>                                                
        ...
        <% } else if(act.equals("Qry")) {%>
            <li>本網頁提供個別農漁會解釋函令查詢功能。</li>
            <li>按<font color="#666666">【查詢】</font>則依查詢條件值查詢資料。</li> 
		    <li>按<font color="#666666">【發文字號】連結</font>則可查看此筆資料。</li>
...
setCheckBox(form.dffeTypeQry,"<%=dffeTypeStr%>");
setCheckBox(form.dffeBTypeQry,"<%=dffeBTypeStr%>");
setText(form.dffeTitle,"<%=dffeTitle%>");
setText(form.dffeContent,"<%=dffeContent%>");
-->
</script>
