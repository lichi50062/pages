...
//103.10.29 fix 結合RptWR003W、RptWR004W、RptWR005W by 2295
//110.09.28 add 取消使用FTP改用SFTP上傳檔案 by 2295   
%>                                                                                         
	        	...
	        	if(!ClientRptDir.exists()){
		  		...
		  		if(errMsg.equals("")){			  		   
         		   ...
         		   if(updateDBList.size() >= 1){         		       
                       ...
                       File rptFile = null;
                       MySFTPClient msftp = new MySFTPClient(rptIP, rptID, rptPwd);//110.09.28 add
                       boolean uploadSuccess = false;//110.09.28 add
                       if(rptDir.exists() && rptDir.isDirectory()){
		                  ...
		                  for(int c=0;c<fname1.length;c++){
		                      ...
		                      uploadSuccess = false;//110.09.28 add
		                  ...
		                  //MyFTPClient ftpC = new MyFTPClient(rptIP, rptID, rptPwd);   //110.09.28
		                  if(S_YEAR.length() == 2){
		                     S_YEAR = "0"+S_YEAR;
		                  } 	     
		                  //putMsg = ftpC.putFiles(Utility.getProperties("serverRptDir"), Utility.getProperties("GenerateRptDir")+System.getProperty("file.separator"),szrpt_code+"/"+S_YEAR+S_MONTH,filename_List);//110.09.28		                  
		                  //ftpC=null;//110.09.28 fix
		                  //if(putMsg == null){//上傳檔案成功//110.09.28 fix                    
		                   if(uploadSuccess){//上傳檔案成功 //110.09.28 add		        
