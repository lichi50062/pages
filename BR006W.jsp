...
//99.12.27 add 查詢日期for100年以後取得新縣市別.100年以前取得舊縣市別 by 2295
...
<%
	...
	if(firstStatus.equals("true")){//若從Menu點選時,先清空session裡的資料
	   session.setAttribute("S_YEAR",null);//99.12.27 add
	   session.setAttribute("S_MONTH",null);//99.12.27 add
    ...
    if(!Utility.CheckPermission(request,report_no)){//無權限時,導向到LoginError.jsp//95.11.03 fix 使用Utility.CheckPermission檢核權限 by 2295
    	...
    	if(!Utility.getTrimString(dataMap.get("SortList")).equals("")){
    	   System.out.println("BR006W.SortList="+(String)request.getParameter("SortList"));
        ...
        if(act.equals("RptColumn")){//報表欄位              
			List FieldList = Utility.getFileData(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"WLX02_M.TXT");						
...
<%!
    private final static String report_no = "BR006W";
    ...
    private String saveReport(HttpServletRequest request,String lguser_id){
    	...
    	try{
        		...
        		File savefile = new File(Utility.getProperties("profileDir")+System.getProperty("file.separator")+ lguser_id+"_BR006W.txt");
        		if(savefile.exists()) savefile.delete();
            	FileOutputStream fos = new FileOutputStream(Utility.getProperties("profileDir")+System.getProperty("file.separator")+ lguser_id+"_BR006W.txt");
    ...
    private String readReport(HttpServletRequest request,String lguser_id){
        ...
        HttpSession session = request.getSession();      
    	try{
    		File WorkFile = new File(Utility.getProperties("profileDir")+System.getProperty("file.separator")+ lguser_id+"_BR006W.txt");	        	
			if(WorkFile.exists()){			
				FileInputStream fis = new FileInputStream(Utility.getProperties("profileDir")+System.getProperty("file.separator")+ lguser_id+"_BR006W.txt");	
