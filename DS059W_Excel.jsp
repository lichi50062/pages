<%
//103.12.02 fix 調整當有新增機構時,無上月逾放比率資料,造成資料無法下載問題 by 2295
...
<%
	...
	try{
			...
			sqlCmd.append("        		field_over_rate,"); //--當月逾期六個月-逾放比率(A)
			sqlCmd.append("        		field_over6m_loan_bal_amt,");//--當月逾期六個月-金額
			sqlCmd.append("        		nvl(field_over_rate_last,0) as field_over_rate_last,");//--上月逾期六個月-逾放比率(B)
			sqlCmd.append("        		field_over6m_loan_bal_amt_last,");//--上月逾期六個月-金額
			sqlCmd.append("        		nvl(field_over_rate,0) - nvl(field_over_rate_last,0) as field_over_rate_diff,");//--逾放比率變動情形(A-B)
			sqlCmd.append("        		'4' as type_over_rate,");
			sqlCmd.append("        		'0' as type_over6m_loan_bal_amt,");
			sqlCmd.append("        		'4' as type_over_rate_last,");
			sqlCmd.append("        		'0' as type_over6m_loan_bal_amt_last,");
			sqlCmd.append("        		'4' as type_over_rate_diff ");
			sqlCmd.append(" 	   from (select agri_loan.m_year,agri_loan.m_month, agri_loan.bank_code ,");
			sqlCmd.append("               		decode(SUM(agri_loan.field_over6m_loan_bal_amt),null,0,SUM(agri_loan.field_over6m_loan_bal_amt)) as field_over6m_loan_bal_amt,");
			sqlCmd.append("               		decode(SUM(agri_loan.field_loan_bal_amt),0,0,round(SUM(agri_loan.field_over6m_loan_bal_amt) /SUM(agri_loan.field_loan_bal_amt)*100 ,2))  as field_over_rate,");
			sqlCmd.append("               		decode(SUM(agri_loan_last.field_over6m_loan_bal_amt),null,0,SUM(agri_loan_last.field_over6m_loan_bal_amt)) as field_over6m_loan_bal_amt_last,");
			sqlCmd.append("               		decode(SUM(agri_loan_last.field_loan_bal_amt),0,0,round(SUM(agri_loan_last.field_over6m_loan_bal_amt) /SUM(agri_loan_last.field_loan_bal_amt)*100 ,2)) as field_over_rate_last ");
			sqlCmd.append("        		   from ");
			sqlCmd.append("        				(select agri_loan.m_year,agri_loan.m_month, agri_loan.bank_code,");        
			sqlCmd.append("                				round(sum(agri_loan.loan_bal_amt) /1,0) as field_loan_bal_amt,");
			sqlCmd.append("                				round(sum(agri_loan.over6m_loan_bal_amt) /1,0) as field_over6m_loan_bal_amt ");                      
            ...
            for(i=0;i<dbData.size();i++){            
                ...
                for(int cellIdx =4;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                     ...
                     if(!amt_type.equals("4")){//該值不為利率或戶數時.再除以單位4捨五入
