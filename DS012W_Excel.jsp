...
<%
   ...
   String printStyle = "";//輸出格式 108.04.03 add 	     
   //輸出格式 108.04.03 add
   ...
   if(act.equals("view")){
      ...
      response.setHeader("Content-disposition","inline; filename=view."+printStyle);//108.04.03調整顯示的副檔名
   }else if (act.equals("download")){
      response.setHeader("Content-Disposition","attachment; filename=download."+printStyle);//108.04.03調整顯示的副檔名
   }
%>
<%
	...
	try{
		...
		wlx01.append(" HSIEN_DIV_1,CANCEL_NO,CANCEL_DATE from wlx01 where m_year =? )");
            ...
            */            
            ...
            String filename = titleName+".xls";//108.04.03 add
            if(!printStyle.equalsIgnoreCase("xls")) {//108.04.03非xls檔須執行轉換	                
	  			rptTrans rptTrans = new rptTrans();	  			
	  			filename = rptTrans.transOutputFormat (printStyle,filename,""); 
	  			System.out.println("newfilename="+filename);	  			   
            };

            FileInputStream fin = new FileInputStream(Utility.getProperties("reportDir")+System.getProperty("file.separator")+ filename);//108.04.03 fix  		 
